module.exports = {
    select: [{
        sql: `SELECT
                  ...
                  (
                      select
                          case
                              when (konto_tyyp ->> 'puhkuse_konto')::boolean then 'PUHKUS'
                              when (konto_tyyp ->> 'haiguse_konto')::boolean then 'HAIGUS'
                              when (konto_tyyp ->> 'lisatasu_konto')::boolean then 'LISATASU'
                              when (konto_tyyp ->> 'koolituse_konto')::boolean then 'KOOLITUS'
                              else 'MUUD'
                              END::varchar(20)
                      from
                  ...
                  case
                      when coalesce((l.properties::jsonb ->> 'puudumise_tyyp'), '')::TEXT = '' then '0'
                      else (l.properties::jsonb ->> 'puudumise_tyyp')::TEXT END::integer as puudumise_tyyp
        ...
        sqlAsNew: `select null::integer as rekvId, 
            ...
            ''::char(1) AS a_req,
            ''::char(1) AS rv_req,
            ''::varchar(20) as puudumise_liik,
            0::integer as  puudumise_tyyp`,
        query: null,
        multiple: false,
        alias: 'row',
        data: []
    ...
    selectAsLibs: `SELECT *,
                   ...
                   FROM
                       com_kontoplaan l
                   ORDER BY
                       kood`, //where ($1::integer is null or l.rekvId = $1 or l.rekvid is null)
    returnData: {
        row: {}
    },
    requiredFields: [
    ...
    deleteDoc: `SELECT
                    ...
                    error_message
                FROM
                    libs.sp_delete_konto($1, $2)`, // $1 - userId, $2 - docId
    grid: {
        gridConfiguration: [
