                  ...
                  FROM
                      (
                          SELECT
                              ...
                              coalesce(j.g31, 0)::NUMERIC(12, 2)                           AS mvt,
                              p.lepingid,
                              t.rekvid,
                              a.id                                                         AS isikid,
                              ...
                              palk.palk_jaak                     j
                                  LEFT OUTER JOIN (
                                                      SELECT
                                                          ...
                                                          sum(p.tka)
                                                          FILTER (WHERE
                                                              (lib.properties :: JSON ->> 'liik') :: INTEGER <> 6 AND
                                                              (lib.properties :: JSON ->> 'liik') :: INTEGER IN
                                                              (1))                                                           AS tka,
                                                          sum(p.tootumaks)
                                                          FILTER (WHERE
                                                              (lib.properties :: JSON ->> 'liik') :: INTEGER <> 6 AND
                                                      ...
                                                      WHERE
                                                            p.lepingid IS NOT NULL
                                                        AND d.doc_type_id IN
                                                            (
                                                                SELECT id
                                                                FROM libs.library l
                                                                WHERE l.library = 'DOK' AND l.kood = 'PALK_OPER'
                                                            )
                                                        AND d.status <> 3
                                                        AND lib.status <> 3
                                                        AND t.status <> 3
