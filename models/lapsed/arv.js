...
const Arv = {
    selectAsLibs: `SELECT *
                   FROM
                       com_arved a
                   WHERE
                       (a.rekvId = $1::INTEGER)`, //$1 - rekvid, $2 userid
    libGridConfig: {
        grid: [
...
--                           LEFT OUTER JOIN lapsed.vanemad v ON v.asutusid = asutus.id
    ...
    select: [
            ...
            sql: `with
                      doc as (
                                 SELECT
                                     ...
                                     (
                                         ...
                                         order by id desc
                                         limit 1
                                     )::text                                                                      AS vana_viitenr,
                                     a.properties ->> 'tyyp'::TEXT                                                AS tyyp,
                                     a.jaak::NUMERIC(12, 2)                                                       AS jaak,
                                     to_char(make_date(year(a.kpv), month(a.kpv), 1)::DATE,
                                             'DD.MM.YYYY')                                                        AS period_alg_print,
                  ...
                  FROM
                      ...
                      (
                          ...
                          FROM
                              doc, lapsed.saldo_ja_kaibeandmik(doc.rekvid,
                                                               make_date(year(doc.doc_kpv), month(doc.doc_kpv), 1)::DATE,
                                                               gomonth(
                                                                       make_date(year(doc.doc_kpv), month(doc.doc_kpv), 01),
                                                                       1)::date - 1, doc.lapsId)
                      ) saldod`,
            sqlAsNew: `SELECT
                           $1 :: INTEGER                                                          AS id,
        ...
        },
        {
            sql: `SELECT
                      ...
                      $2                                                             AS userid
                  FROM
                      docs.arvtasu                       arvtasu
                          INNER JOIN      docs.mk        mk
                                          ON (arvtasu.doc_tasu_id = mk.parentid AND arvtasu.pankkassa = 1)
    ...
    grid: {
        ...
        sqlString: `WITH
                        ...
                        yksused AS (
                                     SELECT
                                         array_to_string(
                                                 public.get_unique_value_from_array(array_agg(a1.properties ->> 'yksus')),
                                                 ',') AS yksus,
                    ...
                    SELECT
                        ...
                        CASE
                            ...
                            WHEN a.ebatoenaolised IS NOT NULL AND a.ebatoenaolised <> '0' AND a.jaak > 0
                                THEN 'Sulgemata'
                            ELSE ''
                        ...
                        case
                            ...
                            (
                                select 1
                                from kreedit_arved
                                where doc_arv_id = a.id
                            ) then 'DB'
                            when exists
                            (
                                select 1
                                from kreedit_arved
                                where doc_tasu_id = a.id
                            ) then 'KR'
                            else '' end as kr_tyyp
                    FROM
                        arved                                           a
                    ...
                    order by
                        a.kpv, yksus`,     //  $1 всегда ид учреждения $2 - всегда ид пользователя
        params: '',
        alias: 'curLasteArved',
        defaults: {
    ...
    deleteDoc: `WITH
                    arved AS (
                                 ...
                                 WHERE
                                     id::TEXT IN (
                                                     SELECT unnest(string_to_array($2::TEXT, ','))
                                                 )
                    )
                SELECT
                    docs.sp_delete_arv($1::INTEGER, id::INTEGER)
                FROM
                    arved
                WHERE
                    CASE WHEN a_len > 1 THEN NOT kas_saldo_ulekanne ELSE TRUE END`, // $1 - userId, $2 - docId
    requiredFields: [
        {
            name: 'kpv',
            type: 'D',
    ...
    koostaKreeditArve: {
        command: `SELECT
                      ...
                      'ARV' AS doc_type_id
                  FROM
                      docs.koosta_kreedit_arve($2::INTEGER, $1::INTEGER)`, //$1 - docs.doc.id, $2 - userId
        type: "sql",
    ...
    generateCashOrder: {
        command: `SELECT
                      ...
                      doc_type_id
                  FROM
                      docs.create_new_order($2::INTEGER, (
                                                             SELECT
                                                                 to_jsonb(row.*)
                                                             FROM
                                                                 (
    ...
    ebatoenaolised: {
        command: `SELECT
                      ...
                      'ARV' AS doc_type_id
                  FROM
                      docs.ebatoenaolised_mahakandmine($2::INTEGER, $1::INTEGER, $3::DATE)`, //$1 - docs.doc.id, $2 - userId, $3 - kpv
        type: "sql",
    ...
    arvestaEbatoenaolised: {
        command: `SELECT
                  ...
                  FROM
                      (
                          SELECT
                              to_jsonb(docs.ebatoenaolised((
                                                               select rekvid
                                                               from ou.userid
                                                               where id = $2::INTEGER
                                                           ), $3::DATE, $1::INTEGER)) tulemus
    ...
    ebatoenaolisedMass: {
        command: `SELECT
                  ...
                  FROM
                      (
                          SELECT
                              to_jsonb(docs.ebatoenaolised_mahakandmine($2::INTEGER, id::INTEGER, $3::DATE)) tulemus
                          FROM
                              cur_arved
                          WHERE
                              id IN (
