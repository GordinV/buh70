...
describe('dok. type Eelarve taitmine aruanne tests', function () {
    ...
    const doc = require('../aruanned/eelarve/eelarve_taitmine'),
        docTypeId = 'EELARVE_TAITMINE'.toLowerCase(),
        modelForExport = 'aruanned/eelarve/eelarve_taitmine';
    ...
    it('doc type library should contain EELARVE_TAITMINE doc.type', async () => {
        let sql = `select id from libs.library where kood = 'EELARVE_TAITMINE' and  library = 'DOK' limit 1`;
    ...
    it('should select data from grid query', async()=> {
        ...
        let returnValue = await db.queryDb(sql, [2018,63, 0]);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
