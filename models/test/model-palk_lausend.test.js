...
describe('dok. type Palk_lausend tests', function () {
    let globalDocId = 0; // для сохранения ид документа

    const doc = require('../aruanned/palk/palk_lausend'),
        docTypeId = 'PALK_LAUSEND'.toLowerCase(),
        modelForExport = 'aruanned/palk/palk_lausend';
    ...
    it('doc type library should contain PALK_LAUSEND doc.type', async () => {
        let sql = `select id from libs.library where kood = 'PALK_LAUSEND' and  library = 'DOK' limit 1`;
    ...
    it('should exists proc palk.palk_lausend', async () => {
        let sql = `select 1 FROM pg_proc WHERE proname = 'palk_lausend'`;
        let returnValue = await db.queryDb(sql, []);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
        expect(result).toBeGreaterThan(0);
    ...
    it('should select data from grid query', async()=> {
        ...
        let returnValue = await db.queryDb(sql, ['2018-01-01','2018-01-31', 1, 0]);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
