...
describe('dok. type kontoasutusandmik_tunnus_tt tests', function () {
    let globalDocId = 0; // для сохранения ид документа

    const doc = require('../aruanned/raamatupidamine/kontosaldoandmik_tunnus_tt'),
        docTypeId = 'KONTOASUTUS_TUN_TT'.toLowerCase(),
        modelForExport = 'aruanned/raamatupidamine/kontosaldoandmik_tunnus_tt';
    ...
    it('doc type library should contain KONTOASUTUSANDMIK TUNNUS tt doc.type', async () => {
        let sql = `select id from libs.library where kood = 'KONTOASUTUS_TUN_TT' and  library = 'DOK' limit 1`;
    ...
    it('should exists proc docs.kontoasutusandmik_tunnus_tt', async () => {
        let sql = `select 1 FROM pg_proc WHERE proname = 'kontoasutusandmik_tunnus_tt'`;
        let returnValue = await db.queryDb(sql, []);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
        expect(result).toBeGreaterThan(0);
    ...
    it('should select data from grid query', async()=> {
        ...
        let returnValue = await db.queryDb(sql, ['201000',null,'2021-01-01','2021-12-31', 3]);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
