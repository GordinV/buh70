...
describe('dok. type Palk_taabel tests', function () {
    let globalDocId = 0; // для сохранения ид документа

    const doc = require('../palk/palk_taabel'),
        docTypeId = 'PALK_TAABEL'.toLowerCase(),
        modelForExport = 'palk/palk_taabel';
...
//        expect(_.find(modelElements.elements, {name:'executeCommand'})).toBeDefined();
        ...
        expect(_.find(gridAlias.elements,{text:'curTaabel1'})).toBeDefined();
    });
    ...
    it('doc type library should contain PALK_TAABEL doc.type', async()=> {
        let sql = `select id from libs.library where kood = 'PALK_TAABEL' and  library = 'DOK' limit 1`;
    ...
    it('should exists view cur_palk_taabel', async()=> {
