...
describe('dok. type TSD lisa 1 tests', function () {
    let globalDocId = 0; // для сохранения ид документа

    const doc = require('../aruanned/palk/tsd_lisa1'),
        docTypeId = 'TSD_LISA1'.toLowerCase(),
        modelForExport = 'aruanned/palk/tsd_lisa1';
    ...
    it('doc type library should contain TSD_LISA1 doc.type', async () => {
    ...
    it('should exists proc palk.tsd_lisa_1', async () => {
        let sql = `select 1 FROM pg_proc WHERE proname = 'tsd_lisa_1'`;
        let returnValue = await db.queryDb(sql, []);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
        expect(result).toBeGreaterThan(0);
