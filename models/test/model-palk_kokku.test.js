...
describe('dok. type Palk_kokku tests', function () {
    let globalDocId = 0; // для сохранения ид документа

    const doc = require('../aruanned/palk/palk_kokku'),
        docTypeId = 'PALK_KOKKU'.toLowerCase(),
        modelForExport = 'aruanned/palk/palk_kokku';
    ...
    it('doc type library should contain PALK_KOKKU doc.type', async () => {
        let sql = `select id from libs.library where kood = 'PALK_KOKKU' and  library = 'DOK' limit 1`;
    ...
    it('should exists proc palk.palk_kokku', async () => {
        let sql = `select 1 FROM pg_proc WHERE proname = 'palk_kokku'`;
        let returnValue = await db.queryDb(sql, []);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
        expect(result).toBeGreaterThan(0);
