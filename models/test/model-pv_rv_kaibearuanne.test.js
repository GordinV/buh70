...
describe('dok. type PV RV kaibe_aruanne tests', function () {
    let globalDocId = 0; // для сохранения ид документа

    const doc = require('../aruanned/pv/pv_rv_kaibearuanne'),
        docTypeId = 'PV_RV_KAIBE_ARUANNE'.toLowerCase(),
        modelForExport = 'aruanned/pv/pv_rv_kaibearuanne';
    ...
    it.skip('doc type library should contain PV_RV_KAIBE_ARUANNE doc.type', async () => {
        let sql = `select id from libs.library where kood = 'PV_RV_KAIBE_ARUANNE' and  library = 'DOK' limit 1`;
    ...
    it.skip('should exists proc docs.pv_kaibe_aruanne', async () => {
        let sql = `select 1 FROM pg_proc WHERE proname = 'pv_rv_kaibe_aruanne'`;
        let returnValue = await db.queryDb(sql, []);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
        expect(result).toBeGreaterThan(0);
