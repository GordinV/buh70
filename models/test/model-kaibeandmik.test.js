...
describe('dok. type kaibeandmik tests', function () {
    let globalDocId = 0; // для сохранения ид документа

    const doc = require('../aruanned/raamatupidamine/kaibeandmik'),
        docTypeId = 'KAIBEANDMIK'.toLowerCase(),
        modelForExport = 'aruanned/raamatupidamine/kaibeandmik';
    ...
    it('doc type library should contain KAIBEANDMIK doc.type', async () => {
        let sql = `select id from libs.library where kood = 'KAIBEANDMIK' and  library = 'DOK' limit 1`;
    ...
    it('should exists proc docs.kaibeandmik', async () => {
        let sql = `select 1 FROM pg_proc WHERE proname = 'kaibeandmik'`;
        let returnValue = await db.queryDb(sql, []);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
        expect(result).toBeGreaterThan(0);
