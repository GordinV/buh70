...
describe('dok. type kassa_raamat tests', function () {
    let globalDocId = 0; // для сохранения ид документа

    const doc = require('../aruanned/raamatupidamine/kassa_raamat'),
        docTypeId = 'KASSA_RAAMAT'.toLowerCase(),
        modelForExport = 'aruanned/raamatupidamine/kassa_raamat';
    ...
    it('doc type library should contain KASSA_RAAMAT doc.type', async () => {
        let sql = `select id from libs.library where kood = 'KASSA_RAAMAT' and  library = 'DOK' limit 1`;
    ...
    it('should exists proc docs.kassa_raamat', async () => {
        let sql = `select 1 FROM pg_proc WHERE proname = 'kassa_raamat'`;
        let returnValue = await db.queryDb(sql, []);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
        expect(result).toBeGreaterThan(0);
