...
describe('dok. type tulud_allikad aruanne tests', function () {
    let globalDocId = 0; // для сохранения ид документа

    const doc = require('../aruanned/eelarve/tulud_allikad'),
        docTypeId = 'TULUD_ALLIKAD'.toLowerCase(),
        modelForExport = 'aruanned/eelarve/tulud_allikad';
    ...
    it('doc type library should contain TULUD_ALLIKAD doc.type', async () => {
        let sql = `select id from libs.library where kood = 'TULUD_ALLIKAD' and  library = 'DOK' limit 1`;
    ...
    it('should select data from grid query', async()=> {
        ...
        let returnValue = await db.queryDb(sql, [2020,'2020-01-01', '2020-08-31', true, 63, 0]);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
