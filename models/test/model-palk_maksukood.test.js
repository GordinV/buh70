...
describe('dok. type maksukood tests', function () {
    let globalDocId = 0; // для сохранения ид документа

    const doc = require('../libs/libraries/maksukood'),
        docTypeId = 'MAKSUKOOD'.toLowerCase(),
        modelForExport = 'libs/libraries/maksukood';
    ...
    it (`${docTypeId} must have fields in xml model`,() => {
        ...
        expect(_.find(gridAlias.elements,{text:'curMaksukood'})).toBeDefined();
    });
    ...
    it('should exists view com_maksukood', async()=> {
        let sql = `select 1 FROM pg_views WHERE viewname = 'com_maksukood'`;
    ...
    it('should return data from  view com_maksukood', async()=> {
        let sql = `select * from palk.com_maksukood`;
        let returnValue = await db.queryDb(sql, []);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
        expect(result).toBeGreaterThan(0);
        expect(returnValue.data.length).toBeGreaterThan(0);
    });
