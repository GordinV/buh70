...
describe('dok. type Pearaamat tests', function () {
    let globalDocId = 0; // для сохранения ид документа

    const doc = require('../aruanned/raamatupidamine/pearaamat'),
        docTypeId = 'PEARAAMAT'.toLowerCase(),
        modelForExport = 'aruanned/raamatupidamine/pearaamat';
    ...
    it('doc type library should contain PEARAAMAT doc.type', async () => {
        let sql = `select id from libs.library where kood = 'PEARAAMAT' and  library = 'DOK' limit 1`;
    ...
    it('should exists proc docs.pearaamat', async () => {
        let sql = `select 1 FROM pg_proc WHERE proname = 'pearaamat'`;
        let returnValue = await db.queryDb(sql, []);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
        expect(result).toBeGreaterThan(0);
    ...
    it('should select data from grid query', async()=> {
        ...
        let returnValue = await db.queryDb(sql, ['113','2018-01-31','2018-08-31', 1]);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
