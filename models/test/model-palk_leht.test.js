...
describe('dok. type Palk_leht tests', function () {
    let globalDocId = 0; // для сохранения ид документа

    const doc = require('../aruanned/palk/palk_leht'),
        docTypeId = 'PALK_LEHT'.toLowerCase(),
        modelForExport = 'aruanned/palk/palk_leht';
    ...
    it('doc type library should contain PALK_LEHT doc.type', async () => {
        let sql = `select id from libs.library where kood = 'PALK_LEHT' and  library = 'DOK' limit 1`;
    ...
    it('should exists proc palk.palk_leht', async () => {
        let sql = `select 1 FROM pg_proc WHERE proname = 'palk_leht'`;
        let returnValue = await db.queryDb(sql, []);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
        expect(result).toBeGreaterThan(0);
