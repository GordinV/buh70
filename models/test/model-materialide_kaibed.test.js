...
describe('dok. type materialide_kaibed tests', function () {
    let globalDocId = 0; // для сохранения ид документа

    const doc = require('../aruanned/ladu/materialide_kaibed'),
        docTypeId = 'MATERIALIDE_KAIBED'.toLowerCase(),
        modelForExport = 'aruanned/ladu/materialide_kaibed';
    ...
    it('doc type library should contain MATERIALIDE_KAIBED doc.type', async () => {
        let sql = `select id from libs.library where kood = 'MATERIALIDE_KAIBED' and  library = 'DOK' limit 1`;
    ...
    it('should exists proc ladu.matkaibed_aruanne', async () => {
        let sql = `select 1 FROM pg_proc WHERE proname = 'matkaibed_aruanne'`;
        let returnValue = await db.queryDb(sql, []);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
        expect(result).toBeGreaterThan(0);
    ...
    it('should select data from grid query', async()=> {
        ...
        let returnValue = await db.queryDb(sql, ['2018-01-01','2018-12-31', 1, null, null]);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
