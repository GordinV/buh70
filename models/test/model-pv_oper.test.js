...
describe('dok. type PV_OPER tests', function () {
    let globalDocId = 0; // для сохранения ид документа

    const doc = require('../pv/pv_oper'),
        docTypeId = 'PV_KAART'.toLowerCase(),
        modelForExport = 'pv/pv_oper';
    ...
    it (`${docTypeId} must have fields in xml model`,() => {
        ...
        expect(_.find(gridAlias.elements,{text:'curPVoper'})).toBeDefined();
        let generateJournal = _.find(modelElements.elements, {name:'generateJournal'});
        expect(generateJournal).toBeDefined();
        let command = _.find(modelElements.elements, {name:'executeCommand'});
