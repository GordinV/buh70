...
describe('dok. type Eelarve taitmine aruanne tests', function () {
    let globalDocId = 0; // для сохранения ид документа

    const doc = require('../aruanned/eelarve/eelarve_kassa_tekkepohine_taitmine'),
        docTypeId = 'KASSA_TEKKE_TAITMINE'.toLowerCase(),
        modelForExport = 'aruanned/eelarve/eelarve_kassa_tekkepohine_taitmine';
    ...
    it('doc type library should contain KASSA_TEKKE_TAITMINE doc.type', async () => {
        let sql = `select id from libs.library where kood = 'KASSA_TEKKE_TAITMINE' and  library = 'DOK' limit 1`;
    ...
    it('should select data from grid query', async()=> {
        ...
        let returnValue = await db.queryDb(sql, ['2018-12-31',1, 0]);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
