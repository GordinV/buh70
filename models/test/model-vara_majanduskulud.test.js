...
describe('dok. type pv_kulud tests', function () {
    let globalDocId = 0; // для сохранения ид документа

    const doc = require('../aruanned/raamatupidamine/vara_majanduskulud'),
        docTypeId = 'PV_KULUD'.toLowerCase(),
        modelForExport = 'aruanned/raamatupidamine/vara_majanduskulud';
    ...
    it.skip('doc type library should contain VARA_MAJANDUSKULUD doc.type', async () => {
        let sql = `select id from libs.library where kood = 'VARA_MAJANDUSKULUD' and  library = 'DOK' limit 1`;
    ...
    it.skip('should exists proc docs.pv_kulud', async () => {
        let sql = `select 1 FROM pg_proc WHERE proname = 'pv_kulud'`;
        let returnValue = await db.queryDb(sql, []);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
        expect(result).toBeGreaterThan(0);
