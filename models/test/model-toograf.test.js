...
describe('dok. type Toograafik tests', function () {
    let globalDocId = 0; // для сохранения ид документа

    const doc = require('../palk/toograf'),
        docTypeId = 'TOOGRAF'.toLowerCase(),
        modelForExport = 'palk/toograf';
    ...
    it (`${docTypeId} must have fields in xml model`,() => {
        ...
        expect(_.find(gridAlias.elements,{text:'curToograf'})).toBeDefined();
    });
    ...
    it('doc type library should contain TOOGRAF doc.type', async()=> {
        let sql = `select id from libs.library where kood = 'TOOGRAF' and  library = 'DOK' limit 1`;
    ...
    it('should exists view cur_toografik', async()=> {
        let sql = `select 1 FROM pg_views WHERE viewname = 'cur_toografik'`;
        let returnValue = await db.queryDb(sql, []);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
        expect(result).toBeGreaterThan(0);
