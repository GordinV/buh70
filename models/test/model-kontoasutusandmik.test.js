...
describe('dok. type kontoasutusandmik tests', function () {
    let globalDocId = 0; // для сохранения ид документа

    const doc = require('../aruanned/raamatupidamine/kontoasutusandmik'),
        docTypeId = 'KONTOASUTUSANDMIK'.toLowerCase(),
        modelForExport = 'aruanned/raamatupidamine/kontoasutusandmik';
    ...
    it.skip('doc type library should contain KONTOASUTUSANDMIK doc.type', async () => {
        let sql = `select id from libs.library where kood = 'KONTOASUTUSANDMIK' and  library = 'DOK' limit 1`;
    ...
    it.skip('should exists proc docs.kontoasutusandmik', async () => {
        let sql = `select 1 FROM pg_proc WHERE proname = 'kontoasutusandmik'`;
        let returnValue = await db.queryDb(sql, []);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
        expect(result).toBeGreaterThan(0);
    ...
    it.skip('should select data from grid query', async()=> {
        ...
        let returnValue = await db.queryDb(sql, ['113',null,'2018-01-01','2018-08-31', 1]);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
