...
describe('dok. type varadearuanne tests', function () {
    let globalDocId = 0; // для сохранения ид документа

    const doc = require('../aruanned/pv/varadearuanne'),
        docTypeId = 'VARADEARUANNE'.toLowerCase(),
        modelForExport = 'aruanned/pv/varadearuanne';
    ...
    it.skip('doc type library should contain VARADEARUANNE doc.type', async () => {
        let sql = `select id from libs.library where kood = 'VARADEARUANNE' and  library = 'DOK' limit 1`;
    ...
    it.skip('should exists proc docs.varadearuanne', async () => {
        let sql = `select 1 FROM pg_proc WHERE proname = 'varadearuanne'`;
        let returnValue = await db.queryDb(sql, []);
        expect(returnValue).toBeDefined();
        let result = returnValue.result;
        expect(result).toBeGreaterThan(0);
