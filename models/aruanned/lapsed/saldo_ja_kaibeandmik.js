module.exports = {
    grid: {
        ...
        sqlString: `WITH report AS (
                        SELECT qryReport.id,
                               qryReport.period,
                               coalesce(alg_db, 0)::NUMERIC(14, 2)     AS alg_db,
                               coalesce(alg_kr, 0)::NUMERIC(14, 2)     AS alg_kr,
                               coalesce(db, 0)::NUMERIC(14, 2)         AS db,
                               coalesce(kr, 0)::NUMERIC(14, 2)         AS kr,
                               coalesce(mahakantud, 0)::NUMERIC(14, 2) AS mahakantud,
                               coalesce(lopp_db, 0)::NUMERIC(14, 2)    AS lopp_db,
                               coalesce(lopp_kr, 0)::NUMERIC(14, 2)    AS lopp_kr,
                               rekvid                                  AS rekv_id,
                               $2                                       AS user_id,
