...
BEGIN
    ...
    THEN
        FOR v_docs IN
            -- libs
            SELECT
                'Lib. Nomenklatuur kood:' || ltrim(rtrim(n.kood)) || ' (' || ltrim(rtrim(r.nimetus)) || ')' AS dok_nr
            FROM
                libs.nomenklatuur      n
                    INNER JOIN ou.rekv r ON n.rekvid = r.id
            WHERE
                  ((n.properties ->> 'valid')::DATE IS NULL OR (n.properties ->> 'valid')::DATE > l_kpv)
            ...
            UNION
            SELECT
                'Lib. amet kood:' || ltrim(rtrim(n.kood)) AS dok_nr
            FROM
                libs.library n
            WHERE
                  n.library = 'AMET'
            ...
            UNION
            SELECT
                'Lib. palk kood:' || ltrim(rtrim(n.kood)) AS dok_nr
            FROM
                libs.library n
            WHERE
                  n.library = 'PALK'
            ...
            UNION
            SELECT DISTINCT
                'Dok. kassa order nr.:' || ltrim(rtrim(m.number)) || ' (' || ltrim(rtrim(r.nimetus)) ||
                ')' AS dok_nr
            FROM
                docs.korder1                m
            ...
            UNION ALL
            SELECT DISTINCT
                'Dok lausend nr.:' || ltrim(rtrim(m.number::TEXT)) || ' (' || ltrim(rtrim(r.nimetus)) ||
                ')' AS dok_nr
            FROM
                cur_journal            m
                    INNER JOIN ou.rekv r ON m.rekvid = r.id
            ...
            LOOP
                tulemus = tulemus || to_jsonb(row)
                          FROM
                              (
                                  SELECT
                                      'Tunnuse kood ' || ltrim(rtrim(l_lib_kood)) || ' kasutusel, ' ||
