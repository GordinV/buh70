DROP FUNCTION IF EXISTS get_rahavoog_kasutus(INTEGER, DATE);

CREATE FUNCTION get_rahavoog_kasutus(l_lib_id INTEGER, l_kpv DATE)
    RETURNS JSONB
    LANGUAGE plpgsql
AS
$$
    ...
    THEN
        FOR v_docs IN
            ...
            WHERE ((n.properties ->> 'valid')::DATE IS NULL OR (n.properties ->> 'valid')::DATE > l_kpv)
              AND n.properties ->> 'rahavoog' IS NOT NULL
              AND ltrim(rtrim(n.properties ->> 'rahavoog')) = l_lib_kood
              AND n.status <> 3
            UNION
            -- docs
            ...
            WHERE o.kpv > l_kpv
              AND ltrim(rtrim(o.kood3)) = l_lib_kood
            UNION ALL
            ...
            FROM cur_journal m
                     INNER JOIN ou.rekv r ON m.rekvid = r.id
            WHERE m.kpv > l_kpv
              AND ltrim(rtrim(m.kood3)) = l_lib_kood
            UNION ALL
            SELECT DISTINCT
            ...
            LOOP
                tulemus = tulemus || to_jsonb(row)
                          FROM (SELECT 'Rahavoog kood ' || ltrim(rtrim(l_lib_kood)) || ' kasutusel, ' ||
...
GRANT EXECUTE ON FUNCTION get_rahavoog_kasutus(INTEGER, DATE) TO dbkasutaja;
GRANT EXECUTE ON FUNCTION get_rahavoog_kasutus(INTEGER, DATE) TO dbpeakasutaja;
