DROP FUNCTION IF EXISTS get_allikas_kasutus(INTEGER, DATE, INTEGER);
DROP FUNCTION IF EXISTS get_allikas_kasutus(INTEGER, DATE);

CREATE FUNCTION get_allikas_kasutus(l_lib_id INTEGER, l_kpv DATE)
    RETURNS JSONB
    LANGUAGE plpgsql
AS
$$
    ...
    THEN
        FOR v_docs IN
            ...
            WHERE ((n.properties ->> 'valid')::DATE IS NULL OR (n.properties ->> 'valid')::DATE > l_kpv)
              AND n.properties ->> 'allikas' IS NOT NULL
              AND ltrim(rtrim(n.properties ->> 'allikas')) = l_lib_kood
            UNION
            -- docs
            ...
            WHERE m.kpv > l_kpv
              AND ltrim(rtrim(m1.kood2)) = l_lib_kood
            UNION
            ...
            WHERE o.kpv > l_kpv
              AND ltrim(rtrim(o.kood2)) = l_lib_kood
            UNION ALL
            ...
            FROM cur_journal m
                     INNER JOIN ou.rekv r ON m.rekvid = r.id
            WHERE m.kpv > l_kpv
              AND ltrim(rtrim(m.kood2)) = l_lib_kood
            UNION ALL
            SELECT DISTINCT 'Dok eelarve taotlus nr.:' || ltrim(rtrim(m.number::TEXT)) || ' (' || ltrim(rtrim(r.nimetus)) ||
                            ')' AS dok_nr
            FROM eelarve.taotlus m
                     INNER JOIN eelarve.taotlus1 m1 ON m.id = m1.parentid
                     INNER JOIN ou.rekv r ON m.rekvid = r.id
            WHERE m.kpv > l_kpv
              AND ltrim(rtrim(m1.kood2)) = l_lib_kood
            ...
            LOOP
                tulemus = tulemus || to_jsonb(row)
                          FROM (SELECT 'Allika kood ' || ltrim(rtrim(l_lib_kood)) || ' kasutusel, ' ||
...
GRANT EXECUTE ON FUNCTION get_allikas_kasutus(INTEGER, DATE) TO dbkasutaja;
GRANT EXECUTE ON FUNCTION get_allikas_kasutus(INTEGER, DATE) TO dbpeakasutaja;
...
SELECT *
FROM jsonb_to_recordset(get_allikas_kasutus(121356, '2019-12-31'::DATE))
