DROP FUNCTION IF EXISTS get_artikkel_kasutus(INTEGER, DATE);

CREATE FUNCTION get_artikkel_kasutus(l_lib_id INTEGER, l_kpv DATE)
    RETURNS JSONB
    LANGUAGE plpgsql
AS
$$
    ...
    THEN
        FOR v_docs IN
            ...
            WHERE ((n.properties ->> 'valid')::DATE IS NULL OR (n.properties ->> 'valid')::DATE > l_kpv)
              AND n.properties ->> 'artikkel' IS NOT NULL
              AND ltrim(rtrim(n.properties ->> 'artikkel')) = l_lib_kood
            UNION
            -- docs
            ...
            WHERE m.kpv > l_kpv
              AND ltrim(rtrim(m1.kood5)) = l_lib_kood
            UNION
            ...
            WHERE o.kpv > l_kpv
              AND ltrim(rtrim(o.kood5)) = l_lib_kood
            UNION ALL
            ...
            FROM cur_journal m
                     INNER JOIN ou.rekv r ON m.rekvid = r.id
            WHERE m.kpv > l_kpv
              AND ltrim(rtrim(m.kood5)) = l_lib_kood
            UNION ALL
            ...
            LOOP
                tulemus = tulemus || to_jsonb(row)
                          FROM (SELECT 'Artikkel kood ' || ltrim(rtrim(l_lib_kood)) || ' kasutusel, ' ||
...
GRANT EXECUTE ON FUNCTION get_artikkel_kasutus(INTEGER, DATE) TO dbkasutaja;
GRANT EXECUTE ON FUNCTION get_artikkel_kasutus(INTEGER, DATE) TO dbpeakasutaja;
