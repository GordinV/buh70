DROP FUNCTION IF EXISTS get_tegevus_kasutus(INTEGER, DATE);

CREATE FUNCTION get_tegevus_kasutus(l_lib_id INTEGER, l_kpv DATE)
    RETURNS JSONB
    LANGUAGE plpgsql
AS
$$
    ...
    THEN
        FOR v_docs IN
            ...
            WHERE ((n.properties ->> 'valid')::DATE IS NULL OR (n.properties ->> 'valid')::DATE > l_kpv)
              AND n.properties ->> 'tegev' IS NOT NULL
              AND ltrim(rtrim(n.properties ->> 'tegev')) = l_lib_kood
            UNION
            -- docs
            ...
            WHERE m.kpv > l_kpv
              AND ltrim(rtrim(m1.kood1)) = l_lib_kood
            UNION
            ...
            WHERE o.kpv > l_kpv
              AND ltrim(rtrim(o.kood1)) = l_lib_kood
            UNION ALL
            ...
            FROM cur_journal m
                     INNER JOIN ou.rekv r ON m.rekvid = r.id
            WHERE m.kpv > l_kpv
              AND ltrim(rtrim(m.kood1)) = l_lib_kood
            UNION ALL
            SELECT DISTINCT
                    'Dok eelarve taotlus nr.:' || ltrim(rtrim(m.number::TEXT)) || ' (' || ltrim(rtrim(r.nimetus)) ||
                    ')' AS dok_nr
            ...
            LOOP
                tulemus = tulemus || to_jsonb(row)
                          FROM (SELECT 'Tegevuse kood ' || ltrim(rtrim(l_lib_kood)) || ' kasutusel, ' ||
...
GRANT EXECUTE ON FUNCTION get_tegevus_kasutus(INTEGER, DATE) TO dbkasutaja;
GRANT EXECUTE ON FUNCTION get_tegevus_kasutus(INTEGER, DATE) TO dbpeakasutaja;
