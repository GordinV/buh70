...
DROP FUNCTION IF EXISTS lapsed.update_arve_dokprop_id(IN arve_id INTEGER, IN rekv_id INTEGER);

CREATE OR REPLACE FUNCTION lapsed.update_arve_dokprop_id(arve_id INTEGER, rekv_id INTEGER)
    RETURNS INTEGER AS
$BODY$
...
SELECT lapsed.update_arve_dokprop_id(a.id, a.rekvid)
FROM docs.arv a
WHERE journalid = 0
  AND kpv = '2020-12-31'
