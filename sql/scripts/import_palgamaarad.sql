...
*/
DROP FUNCTION IF EXISTS import_palgamaarad;

CREATE FUNCTION import_palgamaarad()
    RETURNS INTEGER
    LANGUAGE plpgsql
AS
$$
DECLARE
    v_ameti_klassif RECORD;
    v_maarad        record;
    l_count         INTEGER = 0;
    l_isik_from     integer;
    l_isik_to       integer;
    l_user_id       integer = 2477;
    l_json          jsonb   = '[]';
    v_lib           record;
BEGIN
    for v_ameti_klassif in
        select distinct
            s.kood
        from
            tmp_lib s
        ...
        loop
            ...
            for v_maarad in
                ...
                from
                    tmp_lib t
                where
                    t.kood = v_ameti_klassif.kood
                loop
                    l_json = l_json || jsonb_build_object('palgamaar', v_maarad.palgamaar, 'summa', v_maarad.summa);
                end loop;
            select
                id,
                kood,
            ...
            where
              ...
              and kood = v_ameti_klassif.kood;

            if v_lib is null then
                raise exception 'Kood puudub %', v_ameti_klassif.kood;
            end if;
            l_json =
                    jsonb_build_object('id', v_lib.id, 'kood', v_lib.kood, 'nimetus', v_lib.nimetus, 'library', v_lib.library, 'gridData', l_json);
            l_json = jsonb_build_object('id', v_lib.id, 'data', l_json);
            perform libs.sp_salvesta_ametiklassif(l_json::json, 2477, 63);
            l_json = '[]'::jsonb;
