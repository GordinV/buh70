...
CREATE OR REPLACE FUNCTION docs.gen_lausend_avans(
  IN  tnid          INTEGER,
  IN  userid        INTEGER,
...
BEGIN
  ...
  SELECT
    coalesce(v_avans1.journalid, 0) AS id,
  ...
  LOOP
    ...
    SELECT
      ...
      coalesce(v_avans2.tp, '800599')   AS lisa_d,
      v_dokprop.konto                   AS kreedit,
      coalesce(v_avans2.tp, '800599')   AS lisa_k,
    ...
    l_json_details = l_json_details || CASE WHEN len(l_json_details) > 0
      THEN ','
                                       ELSE '' END || l_json_row;
