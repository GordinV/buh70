...
DROP FUNCTION IF EXISTS rekl.sp_annulleerimine_allkiri(INTEGER, JSON);

CREATE FUNCTION rekl.sp_annulleerimine_allkiri(IN  user_id    INTEGER, IN params JSON, OUT result INTEGER,
...
BEGIN
  ...
  -- устанавливаем статус, снимаем ссылку на проводку
  UPDATE rekl.toiming SET staatus = null, journalid = null WHERE parentid = l_dekl_id;
  ...
  l_json = (SELECT row_to_json(row) FROM (SELECT v_toiming.kasutaja, now() AS updated, 'annulleerimine' as action) row);
...
select rekl.sp_annulleerimine_allkiri(64, '{"id":1443574}')
