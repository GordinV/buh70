...
BEGIN
  ...
  SELECT d.*, t.tyyp, u.ametnik AS user_name, u.roles
      INTO v_doc
  FROM docs.doc d
  ...
  IF NOT (v_doc.roles ->> 'is_rekl_administraator') :: BOOLEAN
  THEN
  ...
  THEN
    ...
    DELETE FROM rekl.ettemaksud WHERE dokid = v_doc.id
                                  AND doktyyp = 'KREEDIT';

    -- меняем статус декларации если ее сальдо = 0

    SELECT t.*, rekl.fnc_dekl_jaak(t.parentid) AS jaak
        INTO v_dekl
    FROM rekl.toiming t
  ...
  SELECT row_to_json(row)
      INTO new_history FROM (SELECT now() AS deleted, v_doc.user_name AS user) row;
