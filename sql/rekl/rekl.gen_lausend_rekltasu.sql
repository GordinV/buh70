DROP FUNCTION IF EXISTS palk.gen_lausend_rekltasu( INTEGER, JSON );
DROP FUNCTION IF EXISTS rekl.gen_lausend_rekltasu( INTEGER, JSON );

CREATE OR REPLACE FUNCTION rekl.gen_lausend_rekltasu(IN  user_id    INTEGER, IN params JSON, OUT result INTEGER,
                                                     OUT error_code INTEGER, OUT error_message TEXT)
  RETURNS RECORD AS
$BODY$
DECLARE
  l_dekl_id   INTEGER = params ->> 'id';
  l_db_konto  TEXT = '200060';
  l_kr_konto  TEXT = '102060';
  l_db_tp     TEXT = '800699';
  l_kr_tp     TEXT = '800699';
  v_toiming   RECORD;
  v_user      RECORD;
...
BEGIN
  ...
  IF (v_toiming.dokpropid IS NOT NULL AND v_toiming.dokpropid = 0) OR v_toiming.summa = 0
  THEN
  ...
  /*
    IF (SELECT t.tyyp
        FROM dekltasu d
          INNER JOIN toiming t ON t.id = d.deklid
        WHERE tasuid = tnId
        LIMIT 1) = 'INTRESS'
    THEN
      l_db_konto = '200095';
  ...
  */
  -- готовим параметры
  SELECT
    v_toiming.journalid                          AS id,
    'JOURNAL'                                    AS doc_type_id,
    v_toiming.kpv                                AS kpv,
    coalesce(v_toiming.selg, 'Reklaam')          AS selg,
    'AUTOMATSELT LAUSEND (GEN_LAUSEND_REKLTASU)' AS muud,
...
GRANT EXECUTE ON FUNCTION rekl.gen_lausend_rekltasu(INTEGER, JSON) TO dbkasutaja;
GRANT EXECUTE ON FUNCTION rekl.gen_lausend_rekltasu(INTEGER, JSON) TO dbpeakasutaja;

/*
SELECT rekl.gen_lausend_rekltasu(1, '{"id": 294174}' :: JSON)
