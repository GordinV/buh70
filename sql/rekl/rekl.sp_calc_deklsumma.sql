...
--    RAISE EXCEPTION 'Vigane period %', l_kpv;
    ...
    THEN
        ...
        THEN
            ...
            -- определяем начало и конеч месяца
...
--        l_lopp_kpv = gomonth(l_alg_kpv, 1) - 1;
            ...
            IF v_luba.algkpv > l_alg_kpv
                AND month(l_kpv) = month(v_luba.algkpv)
                AND year(l_kpv) = year(v_luba.algkpv)
            THEN
                -- esimine kuu
            ...
            THEN
                ...
                IF l_alg_kpv = v_luba.algkpv AND l_lopp_kpv = v_luba.loppkpv
                THEN
                    -- разрешение на месяц, берем сумму месяца
                    l_summa = v_luba.summa;
                END IF;
            END IF; --alg lopp kpv
        --		end if; -- kord
