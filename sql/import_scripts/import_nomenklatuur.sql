...
BEGIN
    ...
    FOR v_nom IN
        ...
        LOOP
            ...
            l_vat = (SELECT CASE
                                WHEN v_nom.doklausid = 0 OR v_nom.doklausid = 5
                                    THEN 20
            ...
            l_user_id = (select id from ou.userid where rekvid = v_nom.rekvid and kasutaja = 'vlad' limit 1);

            RAISE NOTICE 'check for lib.. v_lib.id -> %, found -> % log_id -> %', v_nom.id, nom_id, log_id;

            -- преобразование и получение параметров
            -- сохранение
            SELECT coalesce(nom_id, 0) AS id,
                   v_nom.kood          AS kood,
            ...
            FROM (SELECT coalesce(nom_id, 0) AS id,
                         TRUE                AS import,
                         v_params            AS data) row;

            SELECT libs.sp_salvesta_nomenclature(json_object :: JSON, l_user_id, v_nom.rekvid) INTO nom_id;
            ...
            THEN
                ...
                VALUES (nom_id, v_nom.id, 'NOMENKLATUUR', json_object :: JSON, hist_object :: JSON) RETURNING id
                    INTO log_id;
...
/*
SELECT import_nomenklatuur(id) from nomenklatuur
where rekvid in (
select id from rekv where parentid < 999 and id not in (3,63,131)
)
and id = 27026

select * from rekv where parentid < 999 and id in (3,63,131)
