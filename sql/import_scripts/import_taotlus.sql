...
BEGIN
    ...
    FOR v_taotlus IN
        SELECT t.*
        FROM taotlus t
                 INNER JOIN remote_rekv rekv ON t.rekvid = rekv.id AND rekv.parentid < 999
        ...
        LOOP
            ...
            json_taotlus1 = array_to_json((SELECT array_agg(row_to_json(t1.*))
                                           FROM (SELECT 0                            AS id,
                                                        (SELECT new_id
                                                         FROM import_log
                                                         WHERE old_id = t1.eelarveid
                                                           AND lib_name = 'EELARVE') AS eelarveid,
                                                        (SELECT new_id
                                                         FROM import_log
            ...
            l_koostaja_id = (SELECT new_id
                             FROM import_log
                             WHERE old_id = v_taotlus.koostajaid
                               AND lib_name = 'USERID');
            l_ametnik_id = (SELECT new_id
                            FROM import_log
                            WHERE old_id = v_taotlus.ametnikid
                              AND lib_name = 'USERID');
            l_aktsept_id = (SELECT new_id
                            FROM import_log
                            WHERE old_id = v_taotlus.aktseptid
                              AND lib_name = 'USERID');

            -- сохранение


            SELECT coalesce(taotlus_id, 0) AS id,
            ...
            FROM (SELECT coalesce(taotlus_id, 0) AS id,
                         TRUE                    AS import,
                         v_params                AS data) row;

            SELECT eelarve.sp_salvesta_taotlus(json_object :: JSON, 1, v_taotlus.rekvid) INTO taotlus_id;
            ...
            THEN
                ...
                VALUES (taotlus_id, v_taotlus.id, 'TAOTLUS', json_object :: JSON, hist_object :: JSON) RETURNING id
                    INTO log_id;
            ...
            -- поправить статус
            UPDATE eelarve.taotlus SET status = v_taotlus.staatus WHERE id = taotlus_id;
