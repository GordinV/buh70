...
BEGIN
  ...
  LOOP
    ...
    -- сохранение
    SELECT coalesce(luba_id, 0) AS id,
    ...
    SELECT row_to_json(row)
        INTO json_object
    FROM (SELECT coalesce(luba_id, 0) AS id, TRUE AS import, v_params AS data) row;
...
/*
SELECT import_luba(id) from remote_luba where year(algkpv) >= 2017
...
select * from import_log where old_id = 3044
*/
