...
BEGIN
    ...
    FOR v_leping IN
        ...
        WHERE (l1.id = in_old_id OR in_old_id IS NULL)
            AND (l1.tahtaeg IS NULL
           OR l1.tahtaeg = '2020-12-31')
        LIMIT ALL
        LOOP
            ...
            l_asutus_id = (SELECT new_id
                           FROM import_log
                           WHERE old_id = v_leping.asutusid
                             AND lib_name = 'ASUTUS');
            --objektid
            ...
            ELSE
                l_objekt_id = (SELECT new_id
                               FROM import_log
                               WHERE old_id = v_leping.objektid
                                 AND lib_name = 'OBJEKT');
            ...
            SELECT docs.sp_salvesta_leping(json_object :: JSON, 1, v_leping.rekvid) INTO leping_id;
            ...
            THEN
                ...
                VALUES (leping_id, v_leping.id, 'LEPING', json_object :: JSON, hist_object :: JSON) RETURNING id
                    INTO log_id;
...
/*
SELECT import_lepingud(id) from leping1 where tahtaeg = '2020-12-31'
and number not in (select number from docs.leping1)
and id <> 1232
*/