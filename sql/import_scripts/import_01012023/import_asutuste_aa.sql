DROP FUNCTION IF EXISTS import_asutuste_aa(INTEGER);

CREATE FOREIGN TABLE remote_pank_vv (
    ...
    SERVER db_lapsed
    OPTIONS (SCHEMA_NAME 'lapsed', TABLE_NAME 'pank_vv');



CREATE OR REPLACE FUNCTION import_asutuste_aa(in_old_id INTEGER DEFAULT NULL::INTEGER)
    RETURNS INTEGER AS
$BODY$
DECLARE
    l_asutus_id     INTEGER;
