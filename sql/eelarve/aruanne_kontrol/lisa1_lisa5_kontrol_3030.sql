DROP FUNCTION IF EXISTS eelarve.lisa1_lisa5_kontrol_3030(TEXT, JSON, JSON);

CREATE OR REPLACE FUNCTION eelarve.lisa1_lisa5_kontrol_3030(IN formula TEXT,
                                                            IN params JSON,
                                                            IN data JSON,
                                                            OUT eelarve NUMERIC,
...
--Строка 3030 Kassa eelarve kinn в отчете EELARVEARUANNE (Lisa 1, Lisa 5) -
    -- Сумма всех строк с бюджетом 3030* в отчете Tulude eelarve täitmine (A, TT, RV, Tunnus, Art)  Kassa täitmine=0
        ...
        WHEN l_rekvid_kond <> 999 THEN
            --            Строка 3030 Tekke eelarve kinn в отчете EELARVEARUANNE (Lisa 1, Lisa 5) -
            --            Сумма всех строк с бюджетом 3030* в отчете Tulude eelarve täitmine (A, TT, RV, Tunnus, Art) Tekke eelarve kinn=0

--            Строка 3030 Kassa täitmine (Lisa 5) в отчете EELARVEARUANNE (Lisa 1, Lisa 5) -
            --            Сумма всех строк с бюджетом 3030* в отчете Tulude eelarve täitmine (A, TT, RV, Tunnus, Art) Kassa täitmine +
            --            Сумма всех строк D710001 K 100100 с бюджетом 3030*в Päevaraamat =0
...
SELECT *
FROM eelarve.lisa1_lisa5_kontrol_3030('PÕHITEGEVUSE TULUD KOKKU', '	{
  "eelarve":250602.00,
