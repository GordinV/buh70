DROP FUNCTION IF EXISTS eelarve.lisa1_lisa5_kontrol_art(TEXT, JSON, JSON);

CREATE OR REPLACE FUNCTION eelarve.lisa1_lisa5_kontrol_art(IN formula TEXT,
                                                           IN params JSON,
                                                           IN data JSON,
                                                           OUT eelarve NUMERIC,
...
DECLARE
    ...
    l_art         TEXT    = coalesce((params ->> 'artikkel')::TEXT, '3045');
BEGIN
    ...
    CASE
        WHEN l_rekvid_kond = 999 THEN
            -- Строка 3045 Kassa eelarve kinn в отчете EELARVEARUANNE (Lisa 1, Lisa 5) -
            -- Сумма всех строк с бюджетом 3045* в отчете Tulude eelarve täitmine (A, TT, RV, Tunnus, Art)  Kassa täitmine=0
            ...
            FROM (
                     ...
                     WHERE konto LIKE (ltrim(rtrim(l_art)) +
                                       case when l_art in ('3047', '32', '3220', '3222','3229','3232','3233','3237','3238') then '%' else 'xxx' end)
        ...
        WHEN l_rekvid_kond <> 999 THEN
            -- Строка 3045 Tekke eelarve kinn в отчете EELARVEARUANNE (Lisa 1, Lisa 5) -
            -- Сумма всех строк с бюджетом 3045* в отчете Tulude eelarve täitmine (A, TT, RV, Tunnus, Art) Tekke eelarve kinn=0

--Строка 3045 Kassa täitmine (Lisa 5) в отчете EELARVEARUANNE (Lisa 1, Lisa 5) -
            -- Сумма всех строк с бюджетом 3045* в отчете Tulude eelarve täitmine (A, TT, RV, Tunnus, Art) Kassa täitmine +
            -- Сумма всех строк D710001 K 100100 с бюджетом 3045*в Päevaraamat =0
            WITH qryJournal AS (
                SELECT j.deebet,
                       j.kreedit,
...
SELECT *
FROM eelarve.lisa1_lisa5_kontrol_art('PÕHITEGEVUSE TULUD KOKKU', '	{
  "eelarve":250602.00,
  "eelarve_taps":250602.00,
  "saldoandmik":150706.82,
  "kassa":5664.00,
  "eelarve_kassa":250602.00,
  "rekvid":63,
  "kond": 0,
  "kpv1": 20210101,
  "kpv2": 20210331,
