...
CREATE OR REPLACE FUNCTION eelarve.koosta_kond_saldoandmik(
  IN  user_id       INTEGER,
  IN  l_kpv         DATE,
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO tmp_saldoandmik (nimetus, db, kr, konto, tegev, tp, allikas, rahavoo, timestamp, kpv, rekvid)
      SELECT
        s.nimetus,
        CASE WHEN kontod.tyyp IS NULL OR kontod.tyyp IN (1, 3)
          THEN s.db - s.kr
        ELSE 0 :: NUMERIC END AS db,
        CASE WHEN kontod.tyyp IS NOT NULL AND kontod.tyyp IN (2, 4)
          THEN s.kr - s.db
    ...
    WHERE aasta = year(l_kpv) AND kuu = month(l_kpv)
          ...
          AND ltrim(rtrim(tp)) <> ltrim(rtrim(omatp)) AND
          left(ltrim(rtrim(tp)), 4) = left(ltrim(rtrim(l_Oma_tp)), 4)
    LOOP
  ...
  IF (
       ...
       WHERE ltrim(rtrim(timestamp)) = ltrim(rtrim(l_timestamp)) AND tyyp = 0
             AND left(tp, 4) = left(l_oma_tp, 4) AND left(konto, 1) = '7'

     ) = 0
  THEN
    DELETE FROM tmp_saldoandmik
    WHERE timestamp = l_timestamp AND left(ltrim(rtrim(konto)), 1) = '7';
  ...
  SET nimetus = 'Kassa'
  WHERE konto = '100000' AND ltrim(rtrim(timestamp)) = ltrim(rtrim(l_timestamp));

  -- salvestame kond saldoandmik

  INSERT INTO eelarve.saldoandmik (nimetus, db, kr, konto, tegev, tp, allikas, rahavoo, kpv, aasta, kuu, rekvid, omatp)
    SELECT
      nimetus,
      sum(db),
      sum(kr),
    ...
    FROM (
           SELECT
             ltrim(rtrim(tmp_saldoandmik.nimetus)) AS nimetus,
             CASE WHEN kontod.tyyp IS NULL OR kontod.tyyp IN (1, 3)
             ...
             ltrim(rtrim(tmp_saldoandmik.tp))      AS tp,
