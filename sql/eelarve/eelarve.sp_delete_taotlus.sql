...
BEGIN
    ...
    IF NOT (ou.fnc_aasta_eelarve_kontrol(v_doc.rekvid, v_doc.kpv))
    THEN
    ...
    taotlus_history = row_to_json(row.*)
                      FROM
                          (
                              SELECT
                                  a.*
    ...
    taotlus1_history = jsonb_build_array(array(SELECT
                                                   row_to_json(row.*)
                                               FROM
                                                   (
                                                       SELECT
