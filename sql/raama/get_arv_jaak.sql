DROP FUNCTION IF EXISTS lapsed.get_arv_jaak(INTEGER, DATE);

CREATE OR REPLACE FUNCTION lapsed.get_arv_jaak(l_arv_id INTEGER, l_kpv DATE)
    RETURNS NUMERIC
AS
$BODY$
WITH arve AS (
...
GRANT EXECUTE ON FUNCTION lapsed.get_arv_jaak(INTEGER, DATE) TO dbkasutaja;
GRANT EXECUTE ON FUNCTION lapsed.get_arv_jaak(INTEGER, DATE) TO dbpeakasutaja;
GRANT EXECUTE ON FUNCTION lapsed.get_arv_jaak(INTEGER, DATE) TO dbvaatleja;
GRANT EXECUTE ON FUNCTION lapsed.get_arv_jaak(INTEGER, DATE) TO arvestaja;

SELECT lapsed.get_arv_jaak(5485059, '2024-01-01'::date);
