...
BEGIN
    ...
    THEN
        ...
        doc_number = docs.sp_get_number(user_rekvid, 'TEATIS', YEAR(doc_kpv), l_dokprop_id);
    END IF;
    ...
    THEN
        ...
        VALUES (doc_type_id, '[]' :: JSONB || json_ajalugu, new_rights, user_rekvid, ids)
        RETURNING id
            INTO doc_id;


        raise notice 'doc_kpv %, doc_data ->> kpv %', doc_kpv, doc_data ->> 'kpv';

        INSERT INTO
            docs.teatis (parentid, asutusid, docs, kpv, number, sisu, muud, properties, ajalugu)
        VALUES
            (doc_id, doc_asutusid, ids, doc_kpv, doc_number, doc_sisu, doc_muud,
