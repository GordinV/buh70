...
BEGIN
  ...
  LOOP
    ...
    INTO json_record
    FROM json_to_record(
             json_object) AS x(id TEXT, nomId INTEGER, kogus NUMERIC(14, 4), hind NUMERIC(14, 4), summa NUMERIC(14, 4),
  ...
  WHERE parentid = leping_id AND id NOT IN (SELECT unnest(ids));
...
GRANT EXECUTE ON FUNCTION docs.sp_salvesta_leping(JSON, INTEGER, INTEGER) TO dbkasutaja;
GRANT EXECUTE ON FUNCTION docs.sp_salvesta_leping(JSON, INTEGER, INTEGER) TO dbpeakasutaja;
