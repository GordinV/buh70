...
CREATE OR REPLACE FUNCTION docs.sp_loe_tasu_(IN l_tasu_id INTEGER, IN l_user_id INTEGER,
...
--      AND a.jaak = v_tasu.summa
        ...
        ORDER BY a.kpv, a.id
        LOOP
            RAISE NOTICE 'v_arv.id %, v_arv.jaak %',v_arv.id, v_arv.jaak;
            IF l_tasu_jaak > 0
            THEN
                ...
                RAISE NOTICE 'sp_tasu_arv result %, l_tasu %', result, l_tasu;
...
GRANT EXECUTE ON FUNCTION docs.sp_loe_tasu_(INTEGER, INTEGER) TO dbkasutaja;
GRANT EXECUTE ON FUNCTION docs.sp_loe_tasu_(INTEGER, INTEGER) TO dbpeakasutaja;
GRANT EXECUTE ON FUNCTION docs.sp_loe_tasu_(INTEGER, INTEGER) TO arvestaja;

COMMENT ON FUNCTION docs.sp_loe_tasu_(INTEGER, INTEGER) IS 'производит поиск неоплаченных счетов и вызывает процедуру их оплаты';
