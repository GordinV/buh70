...
DECLARE
    ...
    history_json JSON;
    muud_rigths  JSONB;
    is_eelarve   BOOLEAN = coalesce((new.doc_type_id IN (
        SELECT id
        FROM libs.library
        WHERE (properties :: JSONB ->> 'module') :: TEXT ILIKE '%eelarve%'
          AND library = 'DOK'
    )), FALSE);
    is_rekl      BOOLEAN = coalesce((new.doc_type_id IN (
        SELECT id
        FROM libs.library
        WHERE (properties :: JSONB ->> 'module') :: TEXT ILIKE '%rekl%'
          AND library = 'DOK'
    )), FALSE);
    is_lapsed    BOOLEAN = coalesce((new.doc_type_id IN (
        SELECT id
        FROM libs.library
        ...
        WHEN new.status = 0
            THEN
                ...
                THEN
                    ...
                    SELECT row_to_json(row) INTO muud_rigths
                    FROM (SELECT (CASE
            ...
            THEN
                ...
                FROM (SELECT array(SELECT id
                                   ...
                                   WHERE (kasutaja_ = 1 OR peakasutaja_ = 1 OR
                                     ...
                                     AND rekvid = new.rekvid) AS "update",
                             array(SELECT id
                                   FROM ou.userid
                                   WHERE (kasutaja_ = 1 OR peakasutaja_ = 1 OR
                                          (roles ->> 'is_kasutaja')::BOOLEAN OR
                                          (roles ->> 'is_peakasutaja')::BOOLEAN)
                                     AND rekvid = new.rekvid) AS "delete",
                             array(SELECT id
                                   FROM ou.userid
                ...
                THEN
                    ...
                    FROM (SELECT array(SELECT id
                                       ...
                                       WHERE muud ILIKE '%EelAktsepterja%'
                                         AND rekvid = new.rekvid) AS "EelAktsepterja",
                                 array(SELECT id
                                       FROM ou.userid
                                       WHERE muud ILIKE '%EelKoostaja%'
                                         AND rekvid = new.rekvid) AS "EelKoostaja",
                                 array(SELECT id
                                       FROM ou.userid
                                       WHERE muud ILIKE '%EelAllkirjastaja%'
                                         AND rekvid = new.rekvid) AS "EelAllkirjastaja",
                                 array(SELECT id
                                       FROM ou.userid
                                       WHERE muud ILIKE '%Eelesitaja%'
                                         AND rekvid = new.rekvid) AS "Eelesitaja") row;
                ...
                THEN
                    ...
                    FROM (SELECT array(SELECT id
                                       ...
                                       WHERE muud ILIKE '%reklAdministraator%'
                                         AND rekvid = new.rekvid) AS "reklAdministraator",
                                 array(SELECT id
                                       FROM ou.userid
                                       WHERE muud ILIKE '%reklMaksuhaldur%'
                                         AND rekvid = new.rekvid) AS "reklMaksuhaldur") row;
