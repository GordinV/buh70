...
CREATE OR REPLACE FUNCTION docs.sp_kooperi_arv(
  user_id INTEGER,
  doc_id  INTEGER)
  RETURNS INTEGER AS
$BODY$
...
BEGIN
  ...
  LOOP
    l_details_json = coalesce(l_details_json,'') :: TEXT || case when l_details_json is null then '' else ',' end || row_to_json(v_arvread) :: TEXT;
  END LOOP;
