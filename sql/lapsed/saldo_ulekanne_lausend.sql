...
BEGIN
    ...
    FOR v_saldo IN
        -- получаем сальдо
        SELECT
            (rep.alg_saldo + deebet - kreedit) AS lopp_saldo,
            rep.asutus_id,
            a.tp
        FROM
            docs.kaibeasutusandmik(l_konto, asutus_id_from, l_kpv, l_kpv, l_rekv_id, '%', 0) rep
