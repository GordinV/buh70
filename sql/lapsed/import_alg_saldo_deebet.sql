...
BEGIN
    ...
    FOR json_record IN
        ...
        LOOP
            ...
            if   (l_asutus_id is null or l_laps_id is null) THEN
            ...
            l_user_id = (SELECT id
                         FROM ou.userid WHERE rekvid = l_rekvid AND kasutaja = 'temp');
            ...
            l_arv_id = (SELECT a.parentid
                        FROM docs.doc d
                            ...
                            WHERE kpv = '2019-12-31' AND asutusid = l_asutus_id
            ...
            FOR v_details IN
                ...
                LOOP
                    ...
                    -- ищем номенклатуру
                    SELECT * INTO v_nom
                    FROM libs.nomenklatuur n WHERE rekvid = l_rekvid AND kood = v_details.kood AND n.dok = 'ARV' AND status <> 3 LIMIT 1;
            ...
            RAISE NOTICE 'Arve salvestatud, id-> %',l_arv_id;
...
SELECT error_code, result, error_message
                  FROM lapsed.import_alg_saldo_deebet( '[{"yksus":"0911018","laps_ik":"61611260154","vanem_ik":"49203033730","summa":"14,82","db":"103000","kood":"322020-014","inf3":0}]'::jsonb, 70::integer, 63::integer) as id
