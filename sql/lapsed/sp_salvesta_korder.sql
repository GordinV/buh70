...
BEGIN
    ...
    THEN
        SELECT parentid INTO arv_parent_id
        FROM docs.arv
        WHERE id = doc_arvid;
        IF (SELECT count(*)
            FROM (
...
--        RETURNING id    INTO doc_id;
    ...
    ELSE
        ...
        THEN
            SELECT arvid INTO previous_arv_id
            FROM docs.arv
        ...
        LOOP
            ...
            FROM json_to_record(
                         json_object) AS x (id TEXT, nomid INTEGER, summa NUMERIC(14, 4), nimetus TEXT, tunnus TEXT,
                                            proj TEXT,
                                            konto TEXT, kood1 TEXT, kood2 TEXT, kood3 TEXT, kood4 TEXT, kood5 TEXT,
                                            tp TEXT,
                                            muud TEXT);
            ...
            THEN
                INSERT INTO docs.korder2 (parentid, nomid, nimetus, summa, tunnus, proj, konto, kood1, kood2, kood3,
                                          kood4,
                                          kood5, tp, muud)
                VALUES (korder_id, json_record.nomid, json_record.nimetus, json_record.summa, json_record.tunnus,
                        ...
                        json_record.tp, json_record.muud) RETURNING id
                           INTO korder1_id;
