...
--                 AND (k.properties ->> 'lopp_kpv')::DATE >= '2021-01-01'
                          GROUP BY parentid, rekvid) qry
                    GROUP BY parentid, rekvid) yksus ON yksus.parentid = l.id AND yksus.rekvid = mk.rekvid
              ...
              WHERE d.status <> 3
                AND d.doc_type_id IN
                    (SELECT id FROM libs.library WHERE library.library = 'DOK' AND kood IN ('SMK', 'VMK'))),
     tasud as (
         select lapsed.get_inf3_summa(t.doc_arv_id, t.doc_tasu_id)
         from docs.arvtasu t
         where
     )
select *
