...
BEGIN
    ...
    if l_alus_id is null then
        SELECT
            ...
            rekvid,
            summa,
            docs.sp_update_arv_jaak(a.parentid, v_kreedit_arve.kpv::date) AS jaak,
            l.parentid                                                    AS laps_id
        INTO v_alus_arve
        FROM
            docs.arv                                a
                LEFT OUTER JOIN lapsed.liidestamine l ON l.docid = a.parentid
        WHERE
             a.rekvid = v_kreedit_arve.rekvid
                 AND (a.kpv <= v_kreedit_arve.kpv
                 AND (l.parentid = v_kreedit_arve.laps_id or v_kreedit_arve.laps_id is null)
    ...
    THEN
        ...
        UPDATE docs.arv
        SET
            properties = coalesce(properties, '{}') ::JSONB || jsonb_build_object('kreedit_arve_id', l_arv_id)
        WHERE
            parentid = v_alus_arve.id
        and rekvid = v_alus_arve.rekvid;
        -- кредитовый счет вяжем с основанием

        UPDATE docs.arv
        SET
            properties = coalesce(properties, '{}')::JSONB || jsonb_build_object('alus_arve_id', v_alus_arve.id)
        WHERE
            parentid = l_arv_id
        and rekvid = v_kreedit_arve.rekvid;
