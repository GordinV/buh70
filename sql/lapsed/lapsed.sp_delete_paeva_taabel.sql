...
DROP FUNCTION IF EXISTS lapsed.sp_delete_paeva_taabel(INTEGER, INTEGER);

CREATE OR REPLACE FUNCTION lapsed.sp_delete_paeva_taabel(IN user_id INTEGER,
                                                         IN doc_id INTEGER,
...
BEGIN
    ...
    THEN
        error_code = 6;
        error_message = 'Dokument on kinni, kustutamine on keelatud';
        result = 0;
        RETURN;
    ...
    DELETE FROM lapsed.day_taabel1 WHERE parent_id = doc_id;
...
GRANT EXECUTE ON FUNCTION lapsed.sp_delete_paeva_taabel(INTEGER, INTEGER) TO arvestaja;
GRANT EXECUTE ON FUNCTION lapsed.sp_delete_paeva_taabel(INTEGER, INTEGER) TO dbkasutaja;
GRANT EXECUTE ON FUNCTION lapsed.sp_delete_paeva_taabel(INTEGER, INTEGER) TO dbpeakasutaja;


/*
select lapsed.sp_delete_paeva_taabel(70,21)

SELECT l.*,
           u.ametnik::TEXT                       AS kasutaja,
           (u.roles ->> 'is_arvestaja')::BOOLEAN AS is_arvestaja
    FROM lapsed.day_taabel l
             JOIN ou.userid u ON u.id = 70
    WHERE l.id = 21;
