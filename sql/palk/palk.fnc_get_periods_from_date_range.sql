...
DECLARE
    ...
    l_periods      integer = (select (year(l_lopp_kpv) - year(l_alg_kpv)) * 12 +
...
BEGIN
    raise notice 'l_periods %',l_periods;
    ...
    return query
        select qry.period_start,
               qry.period_finish,
               qry.work_days_in_month,
               qry.work_days_in_period

        from (select (p ->> 'alg_kpv')::date                as period_start,
              ...
              from jsonb_array_elements(tulemused) p) qry;
END;
$BODY$
