...
CREATE OR REPLACE FUNCTION palk.sp_calc_min_sots(IN  user_id INTEGER, IN params JSON,
                                                 OUT result  INTEGER, OUT error_code INTEGER, OUT error_message TEXT,
                                                 OUT summa   NUMERIC, OUT alus NUMERIC)
  RETURNS RECORD AS
$BODY$
DECLARE
...
BEGIN
  ...
  SELECT
    coalesce(v_po.summa, 0)                                 AS summa,
  ...
  l_paevad = (CASE WHEN COALESCE(v_puhkus.puhkuse_paevad, 0) = 0
    THEN 30
...
/*
select * from palk.sp_calc_min_sots(1, '{"lepingid":4}'::JSON);
