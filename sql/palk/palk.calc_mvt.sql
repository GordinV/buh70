...
DROP FUNCTION IF EXISTS palk.calc_mvt(NUMERIC, NUMERIC, DATE);

CREATE OR REPLACE FUNCTION palk.calc_mvt(tulu NUMERIC, mvt NUMERIC, kpv date default current_date)
    RETURNS NUMERIC AS
$BODY$

DECLARE
    l_MVT               NUMERIC(14, 4) = 0;
    l_max_lubatatud_MVT NUMERIC(14, 4) = (
                                             select
                                                 tulubaas
                                             from
                                                 palk.palk_config
    ...
    l_max_MVT           NUMERIC(14, 4) = case
                                             when coalesce(kpv, date(2025, 12, 31)) < '2026-01-01'::date
                                                 then l_max_lubatatud_MVT - l_max_lubatatud_MVT / l_tulu_min_piir *
                                                                            (tulu -
                                                                             l_tulu_max_piir)
                                             else
                                                 l_max_lubatatud_MVT
        end; --500 - 500 / 900 × 	(tulu - 1200) . с 01.01.2026 = 700
...
GRANT EXECUTE ON FUNCTION palk.calc_mvt(NUMERIC, NUMERIC, DATE) TO dbkasutaja;
GRANT EXECUTE ON FUNCTION palk.calc_mvt(NUMERIC, NUMERIC, DATE) TO dbpeakasutaja;

select palk.calc_mvt(3200, 700, '2026-12-31')
