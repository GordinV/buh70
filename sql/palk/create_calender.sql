...
--tahtpaevad
CREATE OR REPLACE FUNCTION public.cur_calender(l_alg_kpv DATE, l_lopp_kpv DATE, l_rekv INTEGER default null)
    RETURNS TABLE
            (
                paev        INTEGER,
                kpv         DATE,
...
WITH
    period AS (
                  SELECT
                      date_part('year', l_alg_kpv)::INTEGER  AS aasta,
...
SELECT
    ...
    (
        ...
        FROM
            cur_tahtpaevad
        WHERE
              aasta = date_part('year', make_date(period.aasta, period.kuu, DAY))
...
FROM
    (
        ...
        FROM
            generate_series(l_alg_kpv::DATE, l_lopp_kpv::DATE, INTERVAL '1' DAY) AS g(dt)
