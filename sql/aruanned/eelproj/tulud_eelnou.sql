             ...
             tmp_andmik AS (
                 ...
                 FROM (
                          SELECT 2               AS tyyp,
                                 ...
                                 12              AS kuu,
                                 qry.rekv_id
                          FROM eelarve.tulu_taitmine(make_date(year(l_kpv) - 1, 1, 1),
...
--  текущий год
                 SELECT 2           AS tyyp,
                        ...
                        06          AS kuu,
                        qry.rekv_id
                 FROM eelarve.tulu_taitmine(make_date(year(l_kpv), 01, 01),
...
-- текущего года seisuga 30.06.2022
                     ...
                     WHERE rekvid = (CASE
                       ...
                       AND e.rekvid IN (SELECT a.rekv_id
                                        FROM get_asutuse_struktuur(l_rekvid) a)
                       AND aasta = YEAR($1)
                       AND (e.kpv IS NULL OR e.kpv <= make_date(YEAR($1)
                         , 06
                         , 30))
                       AND e.status <> 3
                       AND kood5 IN (SELECT kood FROM qryArtikkel)
...
GRANT EXECUTE ON FUNCTION eelarve.tulud_eelnou(DATE, INTEGER, INTEGER, JSONB) TO dbkasutaja;
GRANT EXECUTE ON FUNCTION eelarve.tulud_eelnou(DATE, INTEGER, INTEGER, JSONB) TO dbpeakasutaja;
