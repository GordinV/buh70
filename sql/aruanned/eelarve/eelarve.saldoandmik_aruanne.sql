...
WITH
    ...
    rekv_ids AS (
                     SELECT
                         rekv_id
                     FROM
                         qryParams, public.get_asutuse_struktuur(qryParams.rekvid)
...
--                     l.muud,
            ...
            qrySaldoAndmik AS (
                                  -- alg db kaived
                             SELECT
                                 qryParams.alg_kpv - 1                             AS kpv,
                                 j.rekvid,
                                 j1.deebet                                         AS konto,
--                                 j1.lisa_d                                        AS tp,
                                 case
                                     when
                                         left(j1.deebet, 6) = '155920'
                                             and j.rekvid not in (130, 28)
                                             and year(j.kpv) < 2024
                                         then ''
                                     else
                                         coalesce(j1.lisa_d, '') end:: VARCHAR(20) AS tp,
                                 case
                                     when
                                         left(j1.deebet, 6) = '155920'
                                             and j.rekvid not in (130, 28)
                                         ...
                                         coalesce(j1.kood1, '') end:: VARCHAR(20)  AS tegev,
                                 ...
                                 j1.tunnus,
                                 j1.proj,
                                 array [d.id]::integer[]                           as docs_ids
                             FROM
                                 docs.doc                           d
                             ...
                             --                    AND (j1.deebet = '155920' AND j1.kreedit <> '888888' AND d.rekvid IN (130, 28) OR year(qryParams.kpv) < year(qryParams.kpv) OR j1.deebet <> '155920')
--                             GROUP BY qryParams.alg_kpv, j.rekvid, j1.deebet, j1.lisa_d, j1.kood1, j1.kood2, j1.tunnus,j1.proj
                             UNION ALL
                             -- db kaived

                             SELECT
                                 qryParams.kpv                         AS kpv,
                                 j.rekvid,
                                 j1.deebet                             AS konto,
                                 j1.lisa_d                             AS tp,
...
--                         year(qryParams.kpv) < year(qryParams.kpv) OR j1.deebet <> '155920')
                             GROUP BY
                                 qryParams.kpv, j.rekvid, j1.deebet, j1.lisa_d, j1.kood1, j1.kood2, j1.kood3, j1.tunnus,
                                 j1.proj
...
--                                 coalesce(j1.lisa_k, ''):: VARCHAR(20)            AS tp,
                                 case
                                     when
                                         left(j1.kreedit, 6) = '155920'
                                             and j.rekvid not in (130, 28)
                                             and year(j.kpv) < 2024
                                         then ''
                                     else
                                         coalesce(j1.lisa_k, '') end:: VARCHAR(20) AS tp,

--                                 coalesce(j1.kood1, '') :: VARCHAR(20) AS tegev,
                                 case
                                     when j1.kreedit = '155920'
                                         and d.rekvid not in (130, 28)
                                         and year(j.kpv) < 2024
                                         then ''
                                     else
                                         coalesce(j1.kood1, '') end:: VARCHAR(20)  AS tegev,
...
--                    AND j.kpv <= qryParams.kpv::DATE
                               ...
                               AND coalesce(a.kpv, j.kpv) < qryParams.alg_kpv::DATE
                               AND d.status <> 3
                             --                    AND (j1.kreedit = '155920' AND j1.deebet <> '888888' AND d.rekvid IN (130, 28) OR year(qryParams.kpv) < year(qryParams.kpv) OR j1.kreedit <> '155920')
                             --              GROUP BY qryParams.alg_kpv, j.rekvid, j1.kreedit, j1.lisa_k, j1.kood1, j1.kood2, j1.tunnus,j1.proj
                             UNION ALL
                             -- kr kaived
                             SELECT
                                 qryParams.kpv                        AS kpv,
...
--                    AND coalesce(a.kpv, j.kpv) <= qryParams.kpv::DATE
                               AND coalesce(
                                           a.kpv
                                       , j.kpv) >= qryParams.alg_kpv::DATE
                               AND d.status <> 3
--                    AND (j1.kreedit = '155920' AND j1.deebet <> '888888' AND d.rekvid IN (130, 28) OR year(qryParams.kpv) < year(qryParams.kpv) OR j1.kreedit <> '155920')
                             GROUP BY
                                 qryParams.kpv, j.rekvid, j1.kreedit, j1.lisa_k, j1.kood1, j1.kood2, j1.kood3,
                                 j1.tunnus,
                                 j1.proj
                         )
        SELECT
        ...
        FROM
            (
                SELECT
                    ...
                    konto::TEXT                 AS konto,
                    (CASE
                        -- J.Tsekanina, 20.02.2025 (убрать ранние хвосты)
                         WHEN left(konto, 6) IN ('155920')
                             AND qry.rahavoog = '00'
                             and qry.rekvid in (28)
--                             AND year(qry.kpv) < 2024
                             THEN ''
                         WHEN left(konto, 6) IN ('155920')
                             AND qry.rahavoog = '00'
                             and qry.rekvid in (130, 28)
--                             AND year(qry.kpv) < 2024
                             THEN case when empty(tp) then '800599' else tp end
                         WHEN l.is_tp AND
                              (l.tp_req <> '*' OR
...
--                             AND year(qry.kpv) < 2024
                        ...
                        WHEN left(konto, 6) IN ('155920')
                            AND qry.rahavoog = '00'
                            and qry.rekvid in (130, 28)
                            AND year(qry.kpv) < 2025
                            then case when empty(tegev) then '09110' else tegev end
                        WHEN left(konto, 6) = '155920'
                            AND qry.rahavoog::text = '00'
                            AND year(qry.kpv) < 2024
                            THEN ''
...
SELECT
    ...
    *
FROM
    eelarve.saldoandmik_aruanne('2025-01-01' :: DATE, 28 :: INTEGER, 1 ::integer)
WHERE
    konto like '155920%';
...
FROM eelarve.saldoandmik_aruanne('2025-01-31' :: DATE, 64 :: INTEGER, 1 ::integer)
WHERE konto like '500120%'
--and rahavoog = '01'
--GROUP BY konto, tp
