DROP FUNCTION IF EXISTS eelarve.eelarve_andmik_lisa_1_5_rekvid(DATE, INTEGER, INTEGER);



CREATE OR REPLACE FUNCTION eelarve.eelarve_andmik_lisa_1_5_rekvid(IN l_kpv DATE,
                                                                  IN l_rekvid INTEGER,
                                                                  IN l_kond INTEGER)
    RETURNS TABLE (
        idx                VARCHAR(20),
        ...
        WITH tmp_report AS (
            ...
            FROM (
                     SELECT '2.1'::VARCHAR(20)                                       AS idx,
                            ...
                            get_saldo('KD', '32', NULL, NULL, q.rekvid)::NUMERIC(12, 2)        AS saldoandmik
                     FROM tmp_andmik q
                     ...
                     SELECT '2.1'::VARCHAR(20)                                       AS idx,
                            ...
                            coalesce(coalesce(sum(q.tegelik), 0), 0)::NUMERIC(12, 2) AS tegelik,
                            coalesce(coalesce(sum(q.kassa), 0), 0)::NUMERIC(12, 2)   AS kassa,
                            get_saldo('KD', '320', NULL, NULL, q.rekvid)::NUMERIC(12, 2)       AS saldoandmik
                     FROM tmp_andmik q
                          -- Сумма 320+3220+3221+3222+3224+3229+3232+3233+3237+3238
                     ...
                     SELECT '2.1'::VARCHAR(20),
                            ...
                            'Saadud tegevustoetused'                               AS nimetus,
                            ...
                            get_saldo('KD', '352', NULL, NULL, q.rekvid) -
                            get_saldo('KD', '352000', NULL, NULL, q.rekvid) -
                            get_saldo('KD', '352001', NULL, NULL, q.rekvid)                  AS saldoandmik
...
-- KD352000
                     ...
                     SELECT '2.1'::VARCHAR(20),
                            ...
                            coalesce(sum(q.tegelik), 0)                            AS tegelik,
                            coalesce(sum(q.kassa), 0)                              AS kassa,
                            get_saldo('KD', '352000', NULL, NULL, q.rekvid)                  AS saldoandmik
-- KD352010
                     FROM tmp_andmik q
                     ...
                     SELECT '2.1'::VARCHAR(20),
                            ...
                            coalesce(sum(q.tegelik), 0)                            AS tegelik,
                            coalesce(sum(q.kassa), 0)                              AS kassa,
                            -- KD3823+KD382550+KD382560 (13.09.2022 В. Бешекерскас, новая формула)
--                            KD382-38250-38251-38252-38254
                            get_saldo('KD', '3823', NULL, NULL, q.rekvid)
                                + get_saldo('KD', '382550', NULL, NULL, q.rekvid)
                                + get_saldo('KD', '382560', NULL, NULL, q.rekvid)
                                                                                   AS saldoandmik
-- KD382+KD382520+KD382550+KD382560
                     ...
                     SELECT '2.11'::VARCHAR(20),
                            ...
                            coalesce(sum(q.tegelik), 0)                            AS tegelik,
                            coalesce(sum(q.kassa), 0)                              AS kassa,
                            get_saldo('KD', '40', NULL, NULL, q.rekvid)                      AS saldoandmik
-- KD40
                     FROM tmp_andmik q
                     ...
                     SELECT '2.11'::VARCHAR(20),
                            ...
                            get_saldo('KD', '413', NULL, NULL, q.rekvid)                            AS saldoandmik
-- KD413
                     FROM tmp_andmik q
                     ...
                     SELECT '2.11'::VARCHAR(20),
                            ...
                            -1 * coalesce(sum(q.tegelik), 0)                            AS tegelik,
                            -1 * coalesce(sum(q.kassa), 0)                              AS kassa,
                            get_saldo('KD', '4500', NULL, NULL, q.rekvid)                         AS saldoandmik
-- KD4500
                     FROM tmp_andmik q
                     ...
                     SELECT '2.11'::VARCHAR(20),
                            ...
                            -1 * coalesce(sum(q.tegelik), 0)                            AS tegelik,
                            -1 * coalesce(sum(q.kassa), 0)                              AS kassa,
                            get_saldo('KD', '452', NULL, NULL, q.rekvid)                          AS saldoandmik
-- KD452
                     FROM tmp_andmik q
                     ...
                     SELECT '2.12'::VARCHAR(20),
                            ...
                            -1 * coalesce(sum(q.tegelik), 0)                            AS tegelik,
                            -1 * coalesce(sum(q.kassa), 0)                              AS kassa,
                            get_saldo('KD', '50', NULL, NULL, q.rekvid)                           AS saldoandmik
                     FROM tmp_andmik q
...
-- Строка 55 - Tekke täitmine – добавляем к уже имеющейся формуле KD 55 налог с оборота KD601 минус KD 601002.
                     SELECT '2.12',
                            ...
                            -1 * coalesce(sum(q.tegelik), 0)                            AS tegelik,
                            -1 * coalesce(sum(q.kassa), 0)                              AS kassa,
                            get_saldo('KD', '55', NULL, NULL, q.rekvid) +
                            get_saldo('KD', '601000', NULL, NULL, q.rekvid)                       AS saldoandmik
                     FROM tmp_andmik q
                     ...
                     SELECT '2.13'::VARCHAR(20),
                            ...
                            -1 * coalesce(sum(q.tegelik), 0)                            AS tegelik,
                            -1 * coalesce(sum(q.kassa), 0)                              AS kassa,
                            get_saldo('KD', '60', NULL, NULL, q.rekvid) -
                            get_saldo('KD', '601000', NULL, NULL, q.rekvid) -
                            get_saldo('KD', '601002', NULL, NULL, q.rekvid)                       AS saldoandmik
                     FROM tmp_andmik q
...
--                            KD381-KD3818-KD154RV02-KD155RV02-KD156RV02-KD157RV02-KD109RV02
                            get_saldo('KD', '381', NULL, NULL, q.rekvid) -
                            (get_saldo('KD', '3818', NULL, NULL, q.rekvid) +
                             get_saldo('DK', '154', '02', NULL, q.rekvid) +
                             get_saldo('DK', '155', '02', NULL, q.rekvid) +
                             get_saldo('DK', '156', '02', NULL, q.rekvid) +
                             get_saldo('DK', '157', '02', NULL, q.rekvid) +
                             get_saldo('DK', '109', '02', NULL, q.rekvid))                   AS saldoandmik
-- KD381-KD3818-KD154RV02-KD155RV02-KD156RV02-KD157RV02-KD109RV02
-- KD381-KD3818+KD154RV02+KD155RV02+KD156RV02+KD157RV02+KD109RV02 (c 13.09.2022 поправка)
                     FROM tmp_andmik q
                     ...
                     SELECT q.idx,
                            ...
                            coalesce(sum(q.tegelik), 0)                            AS tegelik,
                            coalesce(sum(q.kassa), 0)                              AS kassa,
                            get_saldo('KD', '3818', NULL, NULL, q.rekvid)                    AS saldoandmik
-- KD381-KD3818-KD154RV02-KD155RV02-KD156RV02-KD157RV02-KD109RV02

                     FROM tmp_andmik q
                     WHERE q.artikkel = '3818'
                       AND q.tyyp = 1
                     GROUP BY q.idx, q.rekvid,q.artikkel,q.nimetus
                     UNION ALL
                     SELECT '2.4.1'::VARCHAR(20),
                            ...
                            (coalesce(get_saldo('KD', '154', '01', NULL, q.rekvid) +
                                      get_saldo('KD', '155', '01', NULL, q.rekvid) +
                                      get_saldo('KD', '156', '01', NULL, q.rekvid) +
                                      get_saldo('KD', '157', '01', NULL, q.rekvid) +
                                      get_saldo('KD', '601002', NULL, NULL, q.rekvid), 0))        AS saldoandmik
                     FROM tmp_andmik q
...
-- KD154RV01+KD155RV01+KD156RV01+KD157RV01+601002KD
                     ...
                     SELECT '2.4.1'::VARCHAR(20),
                            ...
                            coalesce(sum(q.tegelik), 0)                            AS tegelik,
                            coalesce(sum(q.kassa), 0)                              AS kassa,
                            get_saldo('KD', '3502', '01', NULL, q.rekvid) +
                            get_saldo('KD', '3502', '05', NULL, q.rekvid) +
                            get_saldo('KD', '350210', '', NULL, q.rekvid) +
                            get_saldo('KD', '350230', '', NULL, q.rekvid) +
                            get_saldo('KD', '350250', '', NULL, q.rekvid) +
                            get_saldo('KD', '350260', '', NULL, q.rekvid) +
                            get_saldo('KD', '350270', '', NULL, q.rekvid)                    AS saldoandmik
-- KD3502RV01+KD3502RV05
-- KD3502+KD3502RV05+350210+350230+350250+350260+350270 поправка 13.09.2022
                     ...
                     SELECT '2.4.1'::VARCHAR(20),
                            ...
                            -1 * coalesce(sum(q.tegelik), 0)                            AS tegelik,
                            -1 * coalesce(sum(q.kassa), 0)                              AS kassa,
                            get_saldo('KD', '4502', NULL, NULL, q.rekvid) +
                            get_saldo('KD', '1', '24', NULL, q.rekvid)                            AS saldoandmik
-- KD4502+KD1RV24
                     ...
                     SELECT '2.4.2'::VARCHAR(20),
                            ...
                            coalesce(sum(q.tegelik), 0)                            AS tegelik,
                            coalesce(sum(q.kassa), 0)                              AS kassa,
                            get_saldo('KD', '150', '02', NULL, q.rekvid)                     AS saldoandmik
-- KD150RV02
                     ...
                     SELECT '2.4.3',
                            ...
                            -1 * coalesce(sum(q.tegelik), 0)                            AS tegelik,
                            -1 * coalesce(sum(q.kassa), 0)                              AS kassa,
                            get_saldo('KD', '150', '01', NULL, q.rekvid)                          AS saldoandmik
-- KD150RV01
                     FROM tmp_andmik q
                     ...
                     SELECT '2.4.3',
                            ...
                            coalesce(sum(q.tegelik), 0)                            AS tegelik,
                            coalesce(sum(q.kassa), 0)                              AS kassa,
                            get_saldo('KD', '151910', '02', NULL, q.rekvid) +
                            get_saldo('KD', '101900', '02', NULL, q.rekvid)                  AS saldoandmik
-- KD151910RV02+KD101900RV02
                     ...
                     SELECT '2.4.4',
                            ...
                            coalesce(sum(q.tegelik), 0)                            AS tegelik,
                            coalesce(sum(q.kassa), 0)                              AS kassa,
                            get_saldo('KD', '151910', '01', NULL, q.rekvid) +
                            get_saldo('KD', '101900', '01', NULL, q.rekvid)                  AS saldoandmik
-- KD151910RV01+KD101900RV01
                     ...
                     SELECT '2.4.4',
                            ...
                            coalesce(sum(q.tegelik), 0)                            AS tegelik,
                            coalesce(sum(q.kassa), 0)                              AS kassa,
                            get_saldo('KD', '1032', '02', NULL, q.rekvid) +
                            get_saldo('KD', '1532', '02', NULL, q.rekvid)                    AS saldoandmik
-- KD1032RV02+KD1532RV02
                     ...
                     SELECT '2.4.5',
                            ...
                            coalesce(sum(q.tegelik), 0)                            AS tegelik,
                            coalesce(sum(q.kassa), 0)                              AS kassa,
                            get_saldo('KD', '1032', '01', NULL, q.rekvid) +
                            get_saldo('KD', '1532', '01', NULL, q.rekvid)                    AS saldoandmik
-- KD1032RV01+KD1532RV01
                     ...
                     SELECT '2.4.5',
                            ...
                            coalesce(sum(q.tegelik), 0)                            AS tegelik,
                            coalesce(sum(q.kassa), 0)                              AS kassa,
                            get_saldo('KD', '652', NULL, NULL, q.rekvid) -
                            get_saldo('KD', '652000', NULL, NULL, q.rekvid) -
                            get_saldo('KD', '652030', NULL, NULL, q.rekvid) +
                            get_saldo('KD', '655', NULL, NULL, q.rekvid) +
                            get_saldo('KD', '658', NULL, NULL, q.rekvid) -
                            get_saldo('KD', '658950', NULL, NULL, q.rekvid) -
                            get_saldo('KD', '655400', NULL, NULL, q.rekvid)                  AS saldoandmik
-- KD652+KD655+KD658-KD658950
                     ...
                     SELECT '2.4.5',
                            ...
                            -1 * coalesce(sum(q.tegelik), 0)                            AS tegelik,
                            -1 * coalesce(sum(q.kassa), 0)                              AS kassa,
                            get_saldo('KD', '650', NULL, NULL, q.rekvid) +
                            get_saldo('KD', '658950', NULL, NULL, q.rekvid)                       AS saldoandmik
-- KD650+KD658950
                     FROM tmp_andmik q
                     ...
                     SELECT '2.4.6',
                            ...
                            coalesce(sum(q.tegelik), 0)                            AS tegelik,
                            coalesce(sum(q.kassa), 0)                              AS kassa,
                            get_saldo('KD', '208', '05', NULL, q.rekvid) +
                            get_saldo('KD', '258', '05', NULL, q.rekvid)                     AS saldoandmik
-- KD208RV05+KD258RV05
                     FROM tmp_andmik q
                     ...
                     SELECT '2.4.6',
                            ...
                            'Kohustiste tasumine (-)'                                   AS nimetus,
                            -1 * coalesce(sum(q.eelarve), 0)                            AS eelarve,
                            -1 * coalesce(sum(q.eelarve_kassa), 0)                      AS eelarve_kassa,
                            -1 * coalesce(sum(q.eelarve_taps), 0)::NUMERIC(12, 2)       AS eelarve_taps,
                            -1 * coalesce(sum(q.eelarve_kassa_taps), 0)::NUMERIC(12, 2) AS eelarve_kassa_taps,
                            -1 * coalesce(sum(q.tegelik), 0)                            AS tegelik,
--                            -1 * coalesce(sum(q.kassa), 0)                              AS kassa,
                            -1 * (select sum(j1.summa) as summa
                                       from docs.doc d
                                       inner join docs.journal j on j.parentid = d.id
                                       inner join docs.journal1 j1 on j1.parentid = j.id
                                         WHERE d.rekvid = q.rekvid
                                           AND (j1.kreedit LIKE '100%' OR j1.kreedit LIKE '999%')
                                           AND j1.kood5 = '2586'
                                           AND year(j.kpv) = year(l_kpv)
                                           AND j.kpv <= l_kpv
                                       )  AS kassa,
                            get_saldo('KD', '208', '06', NULL, q.rekvid) +
                            get_saldo('KD', '258', '06', NULL, q.rekvid)                          AS saldoandmik
-- KD208RV06+KD258RV06
                     ...
                     SELECT '2.4.7',
                            ...
                            -1 * coalesce(sum(q.eelarve_kassa_taps), 0)::NUMERIC(12, 2) AS eelarve_kassa_taps,
                            -1 * coalesce(sum(q.tegelik), 0)                            AS tegelik,
                            (get_saldo('DK', '100', NULL, NULL, q.rekvid) -
                             get_saldo('DK', '100080', NULL, NULL, q.rekvid)) -
                            (get_saldo('MDK', '100', NULL, NULL, q.rekvid) -
                             get_saldo('MDK', '100080', NULL, NULL, q.rekvid))                    AS kassa,
--                            DK100-MDK100+DK101-MDK101-DK1019+MDK1019+DK151-MDK151-DK1519+MDK1519
                            get_saldo('DK', '100', NULL, NULL, q.rekvid)
                                - get_saldo('MDK', '100', NULL, NULL, q.rekvid) +
                            get_saldo('DK', '101', NULL, NULL, q.rekvid) -
                            get_saldo('MDK', '101', NULL, NULL, q.rekvid) -
                            get_saldo('DK', '1019', NULL, NULL, q.rekvid) +
                            get_saldo('MDK', '1019', NULL, NULL, q.rekvid) +
                            get_saldo('DK', '151', NULL, NULL, q.rekvid) -
                            get_saldo('MDK', '151', NULL, NULL, q.rekvid) -
                            get_saldo('DK', '1519', NULL, NULL, q.rekvid) +
                            get_saldo('MDK', '1519', NULL, NULL, q.rekvid)
                                                                                        AS saldoandmik
-- DK100-MDK100+DK101-MDK101-DK1019+MDK1019+DK151-MDK151-DK1519+MDK1519
...
-- KD910090
                     ...
                     SELECT q.idx,
                            ...
                            COALESCE(sum(q.tegelik), 0)            AS tegelik,
                            COALESCE(sum(q.kassa), 0)              AS kassa,
                            get_saldo('KD', q.artikkel, NULL, NULL, q.rekvid)  AS saldoandmik
                     FROM tmp_andmik q
                     ...
                     FROM (
                              SELECT '3.1'::VARCHAR(20)   AS idx,
                                     ...
                                     COALESCE(sum(CASE
                                                      ...
                                                      ELSE qry.kassa END),
                                              0)          AS kassa,
                                     (get_saldo('DK', '4', NULL, qry.tegev, qry.rekvid) +
                                      get_saldo('DK', '5', NULL, qry.tegev, qry.rekvid) +
                                      get_saldo('DK', '6', NULL, qry.tegev, qry.rekvid) +
                                      get_saldo('DK', '15', '01', qry.tegev, qry.rekvid) -
                                      get_saldo(
                                              'DK',
                                              '610', NULL, qry.tegev, qry.rekvid) -
                                      get_saldo(
                                              'DK',
                                              '611', NULL, qry.tegev, qry.rekvid) -
                                      get_saldo(
                                              'DK',
                                              '613', NULL,
                                              qry.tegev, qry.rekvid)) AS saldoandmik
                              FROM tmp_andmik qry
                              ...
                              GROUP BY qry.tegev, l.nimetus, qry.rekvid
                              UNION ALL
                              ...
                              SELECT '3.1'::VARCHAR(20)                  AS idx,
                                     ...
                                     0                                   AS tegelik,
                                     0                                   AS kassa,
                                     get_saldo('DK', '608000', NULL) +
                                     get_saldo('DK', '6012', NULL, NULL) +
                                     get_saldo('DK', '6015', NULL, NULL) +
                                     get_saldo('DK', '101900', '01', NULL) +
                                     get_saldo('DK', '1032', '01', NULL) +
                              ...
                              SELECT '3.1.099'::VARCHAR(20)               AS idx,
                                     ...
                                     ''::VARCHAR(20)                      AS allikas,
                                     ''::VARCHAR(20)                      AS artikkel,
                                     'Korrigeerimine  (kõik tegevusalad)' AS nimetus,
                                     0                                    AS eelarve,
                                     0                                    AS eelarve_kassa,
                                     0                                    AS eelarve_taps,
                                     0                                    AS eelarve_kassa_taps,
                                     0                                    AS tegelik,
                                     0                                    AS kassa,
                                     get_saldo('DK', '1', '24', NULL)     AS saldoandmik
                          ) qry
...
GRANT EXECUTE ON FUNCTION eelarve.eelarve_andmik_lisa_1_5_rekvid(DATE, INTEGER, INTEGER ) TO dbpeakasutaja;
GRANT EXECUTE ON FUNCTION eelarve.eelarve_andmik_lisa_1_5_rekvid(DATE, INTEGER, INTEGER ) TO eelaktsepterja;
GRANT EXECUTE ON FUNCTION eelarve.eelarve_andmik_lisa_1_5_rekvid(DATE, INTEGER, INTEGER ) TO dbvaatleja;


SELECT *
FROM eelarve.eelarve_andmik_lisa_1_5_rekvid(DATE(2023,08, 31),119, 0) qry
...
select get_saldo('KD', '32', '00', NULL) ,
