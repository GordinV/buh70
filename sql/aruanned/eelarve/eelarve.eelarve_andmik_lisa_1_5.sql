                          ...
                          coalesce((SELECT sum(summa) AS summa
                                    FROM cur_journal j
                                        WHERE j.rekvid = (CASE
                                             ...
                                             AND (j.deebet LIKE '100%' OR j.deebet = '999999')
                                             AND j.kood5 = '352'
                                             AND j.kpv <= l_kpv
                                   ), 0) -
                          coalesce((SELECT sum(summa) AS summa
 ...
 */
                         ...
                         SELECT
                          ...
                          sum(q.eelarve) AS eelarve_kassa,
                          sum(q.eelarve_taps) AS eelarve_taps,
                          sum(q.eelarve_taps) AS eelarve_kassa_taps,
                          -1 * get_saldo('KD', '910090', NULL, NULL) AS tegelik,
                          -1 * get_saldo('KD', '910090', NULL, NULL) AS kassa,
                          -1 * get_saldo('KD', '910090', NULL, NULL) AS saldoandmik
                          from tmp_andmik q
...
-- KD910090
                         ...
                         WHERE
                          left(q.artikkel, 2) NOT IN ('15', '40', '50', '55', '60', '91')
                              AND left(q.artikkel, 4) NOT IN
                                  ('3200', '3201', '3203', '3209', '3502', '3823', '2585', '2586', '1001', '4500',
                                   '4502')
                              AND left(q.artikkel, 3) NOT IN ('655', '650', '352', '381', '413', '452', '910', '382')
                              AND tyyp = 1
                         GROUP BY
                          q.idx,
                          q.artikkel,
                         ...
                         SELECT
                          ...
                          (get_saldo('DK', '4', NULL, qry.tegev) +
                           ...
                           get_saldo('DK', '15', '02', qry.tegev) -
                           get_saldo('DK', '15', '19', qry.tegev) -
                           get_saldo('DK', '15', '17', qry.tegev) -
                         ...
                         FROM
                          tmp_andmik qry
                              LEFT OUTER JOIN libs.library l ON l.kood = qry.tegev
                              AND l.library = 'TEGEV'
                              AND l.status <> 3
                         WHERE
                          tyyp = 1
            ...
            FROM
             (
                 WITH qryKassa AS (
                     ...
                     SELECT coalesce((
                                         SELECT sum(
                                                        (CASE WHEN left(j1.deebet, 3) = '100' THEN 1 ELSE -1 END) * j1.summa)
                                         FROM docs.doc d
                 ...
                 SELECT '2.1',
                        ...
                        0                                          AS eelarve,
                        sum(t.eelarve_kassa) - sum(qryKassa.kassa) AS eelarve_kassa,
                        0                                          AS eelarve_taps,
                        sum((((CASE WHEN t.eelarve_taps IS NULL THEN 0 ELSE t.eelarve_taps END) +
                              (CASE WHEN t.eelarve IS NULL THEN 0 ELSE t.eelarve END))::NUMERIC)) -
                        sum(qryKassa.kassa)                        AS eelarve_kassa_taps,
                        0                                          AS tegelik,
                        sum(qryKassa.kassa)                        AS kassa,
                        0                                          AS saldoandmik
                 FROM tmp_andmik t
                          INNER JOIN libs.library l
                                     ON t.artikkel = l.kood
