...
DECLARE
    ...
    la_kontod       TEXT[]         = ARRAY ['100','1000','1001','15','1501','1502','1511','1512','1531','1532','2580','2581','2585','2586','3000','3030',
        ...
        '38252','38253','38254','3880','3882','3888','40','413','4500','4502','452','50','55','60','650','655'];
...
BEGIN
    ...
    FROM
        ...
        (
            SELECT
                min(aasta) AS eelmine_aasta,
                max(aasta) AS aasta,
                min(kuu)   AS eelmine_kuu,
                max(kuu)   AS kuu
            FROM
                tmp_andmik
        )          aasta
                              ...
                              FROM
                                  (
                                      ...
                                      WHERE
                                            (
                                                q.artikkel IN
                                                ('3220', '3221', '3222', '3223', '3224', '3229', '3232', '3233', '3237',
                                                 '3238')
...
*/
                                          get_saldo('KD', '3823', NULL, NULL)
                                              + get_saldo('KD', '382550', NULL, NULL)
                                              + get_saldo('KD', '382560', NULL, NULL)
                                                                                                 AS saldoandmik
-- KD382+KD382520+KD382550+KD382560
                                      ...
                                      SELECT
                                          ...
                                          ''::VARCHAR(20)                                        AS tegev,
                                          ''::VARCHAR(20)                                        AS allikas,
                                          '38254'::VARCHAR(20)                                   AS artikkel,
                                          'Laekumine vee erikasutusest'                          AS nimetus,
                                          coalesce(sum(q.eelarve), 0)                            AS eelarve,
                                          coalesce(sum(q.eelarve_kassa), 0)                      AS eelarve_kassa,
                                          coalesce(sum(q.eelarve_taps), 0)::NUMERIC(12, 2)       AS eelarve_taps,
                                          coalesce(sum(q.eelarve_kassa_taps), 0)::NUMERIC(12, 2) AS eelarve_kassa_taps,
                                          coalesce(sum(q.tegelik), 0)                            AS tegelik,
                                          coalesce(sum(q.kassa), 0)                              AS kassa,
                                          get_saldo('KD', '38253', NULL, NULL)
                                              + get_saldo('KD', '38254', NULL, NULL)             AS saldoandmik

                                      FROM
                                          tmp_andmik q
                                      WHERE
                                            q.artikkel LIKE '38254%'
                                        AND tyyp = 1
                                      UNION ALL
                                      SELECT
                                          '2.11'::VARCHAR(20),
...
-- KD154RV01+KD155RV01+KD156RV01+KD157RV01+601002KD
                                      ...
                                      SELECT
                                          ...
                                          get_saldo('KD', '350210', '', NULL) +
                                          get_saldo('KD', '350230', '', NULL) +
                                          get_saldo('KD', '350250', '', NULL) +
                                          get_saldo('KD', '350260', '', NULL) +
                                          get_saldo('KD', '350270', '', NULL)                    AS saldoandmik
...
-- KD1032RV02+KD1532RV02
                                      FROM
                                          tmp_andmik q
                                      WHERE
                                            left(q.artikkel, 4) IN ('1032', '1532')
                                        AND tyyp = 1
                                      UNION ALL
                                      SELECT
                                          '2.4.5'
...
-- KD208RV05+KD258RV05
                                      FROM
                                          tmp_andmik q
                                      WHERE
                                            (left(q.artikkel, 3) = '255' OR left(q.artikkel, 4) = '2585')
                                        AND tyyp = 1
                                      UNION ALL
                                      SELECT
                                          '2.4.6',
                                                    ...
                                                    WHERE
                                                      ...
                                                      AND j.rekvid IN (
                                                                          SELECT
                                                                              rekv_id
                                                                          FROM
                                                                              get_asutuse_struktuur(l_rekvid, l_kpv)
                                                                      )
...
-- KD208RV06+KD258RV06
                                      ...
                                      FROM
                                          tmp_andmik q
                                      WHERE
                                            (left(q.artikkel, 3) = '206' OR left(q.artikkel, 4) = '2586')
                                        AND tyyp = 1
                                      UNION ALL
                                      SELECT
                                          '2.4.7'
...
-- DK100-MDK100+DK101-MDK101-DK1019+MDK1019+DK151-MDK151-DK1519+MDK1519
                                      ...
                                      FROM
                                          tmp_andmik q
                                      WHERE
                                            (q.artikkel LIKE '1001.%' OR q.artikkel = '100')
                                        AND tyyp = 1
                                      UNION ALL
                                      SELECT
                                          '8.11',
...
-- KD910090
                                      ...
                                      FROM
                                          tmp_andmik q
                                      WHERE
                                            LEFT(q.artikkel, 2) NOT IN ('15', '40', '50', '55', '60', '91')
                                        AND LEFT(q.artikkel, 4) NOT IN (
                                                                        ...
                                                                        '4500',
                                                                        '4502')
                                        AND LEFT(q.artikkel, 3) NOT IN
                                            ('655', '650', '352', '381', '413', '452', '910', '320')
                                        AND trim(q.artikkel) NOT IN ('382', '100', '38254')
                                        AND tyyp = 1
                                      GROUP BY
                                          q.idx,
                                          q.artikkel,
                                      ...
                                      GROUP BY qry.idx, qry.is_e, qry.rekvid, qry.tegev, qry.allikas, qry.artikkel,
                                               qry.nimetus
                                  ) qry
                              WHERE
                                  qry.artikkel NOT IN ('3500.00', '3209')
            ) -- VB, нет необходимости
        SELECT *
        FROM
            tmp_report
        WHERE
            tmp_report.artikkel NOT IN ('6580')
        UNION ALL
                ...
                WITH
                    qryEelarve AS (
                                      SELECT
                                          ...
                                          (
                                              coalesce(
                                                              sum(e.summa_kassa) FILTER (WHERE e.is_kulud > 0 AND e.kpv IS NULL),
                                                              0)
                                          ...
                                          (
                                              coalesce(sum(e.summa)
                                                       FILTER (WHERE e.is_kulud > 0 AND e.kpv IS NOT NULL AND e.kpv <= l_kpv),
                                                       0)
                                                  - coalesce(sum(e.summa)
