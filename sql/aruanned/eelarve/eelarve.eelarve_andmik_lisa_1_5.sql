...
BEGIN
    ...
    SELECT sum(tmp_andmik.kassa) INTO l_3888
    FROM tmp_andmik WHERE tmp_andmik.artikkel::TEXT = '3888';
...
-- data analise
    RETURN QUERY
        SELECT qry.idx::VARCHAR(20),
               ...
               (CASE WHEN qry.eelarve_kassa IS NULL THEN 0 ELSE qry.eelarve_kassa END)::NUMERIC + (CASE
        ...
        FROM (
                 ...
                 FROM tmp_andmik q
                     ...
                     WHERE
                      (
                                  q.artikkel IN ('3220', '3221', '3222', '3224', '3229', '3232', '3233', '3237', '3238')
...
-- KD352010
                     ...
                     SELECT
                      ...
                      coalesce(sum(q.kassa), 0) AS kassa
                         ,
                      get_saldo('KD', '382', NULL, NULL) AS saldoandmik
...
-- KD154RV01+KD155RV01+KD156RV01+KD157RV01+601002KD
                     ...
                     SELECT
                      ...
                      get_saldo('KD', '3502', '05', NULL) + get_saldo('KD', '3502', '', NULL) AS saldoandmik
...
-- @TODO oodan selesused
                     ...
                     SELECT
                      ...
                      get_saldo('MKD', '208', NULL, NULL) + get_saldo('MKD', '258', NULL, NULL) AS eelarve_kassa,
                      get_saldo('MKD', '208', NULL, NULL) + get_saldo('MKD', '258', NULL, NULL) AS eelarve_taps,
                      get_saldo('MKD', '208', NULL, NULL) + get_saldo('MKD', '258', NULL, NULL) AS eelarve_kassa_taps,
                      coalesce(sum(q.tegelik), 0) AS tegelik,
                      coalesce(sum(q.kassa), 0) AS kassa,
...
-- MKD910090
                     ...
                     SELECT
                      ...
                      get_saldo('MDK', '1019', NULL, NULL) +
                      get_saldo('MDK', '151', NULL, NULL) -
                      get_saldo('MDK', '1519', NULL, NULL) AS eelarve_taps,
                      get_saldo('MDK', '100', NULL, NULL) +
                      get_saldo('MDK', '101', NULL, NULL) -
                      get_saldo('MDK', '1019', NULL, NULL) +
                      get_saldo('MDK', '151', NULL, NULL) -
                      get_saldo('MDK', '1519', NULL, NULL) AS eelarve_kassa_taps,
                      0 AS tegelik
                         ,
                      0 AS kassa
                         ,
                     ...
                     SELECT
                      ...
                      (SELECT sum(q.eelarve)
                       FROM tmp_andmik q
                           WHERE (left(q.artikkel, 3) = '255' OR left(q.artikkel, 4) = '2585')
                                AND tyyp = 1) +
                      (SELECT sum(-1 * q.eelarve_kassa)
                       FROM tmp_andmik q
                           WHERE (left(q.artikkel, 3) = '206' OR left(q.artikkel, 4) = '2586')
                                AND tyyp = 1) +
                      get_saldo('MKD', '208', NULL, NULL) +
                      get_saldo('MKD', '258', NULL, NULL) AS eelarve_kassa,

                      (SELECT sum(q.eelarve_taps)
                       FROM tmp_andmik q
                           WHERE (left(q.artikkel, 3) = '255' OR left(q.artikkel, 4) = '2585')
                                AND tyyp = 1) +
                      (SELECT sum(-1 * q.eelarve_taps)
                       FROM tmp_andmik q
                           WHERE (left(q.artikkel, 3) = '206' OR left(q.artikkel, 4) = '2586')
                                AND tyyp = 1) +
                      get_saldo('MKD', '208', NULL, NULL) +
                      get_saldo('MKD', '258', NULL, NULL) AS eelarve_taps,
                      (SELECT sum(q.eelarve_kassa_taps)
                       FROM tmp_andmik q
                           WHERE (left(q.artikkel, 3) = '255' OR left(q.artikkel, 4) = '2585')
                                AND tyyp = 1) +
                      (SELECT sum(-1 * q.eelarve_kassa_taps)
                       FROM tmp_andmik q
                           WHERE (left(q.artikkel, 3) = '206' OR left(q.artikkel, 4) = '2586')
                                AND tyyp = 1) +
                      get_saldo('MKD', '208', NULL, NULL) +
                      get_saldo('MKD', '258', NULL, NULL) AS eelarve_kassa_taps,
...
-- pohi osa
                     ...
                     FROM
                      tmp_andmik q
                     WHERE
                      left(q.artikkel, 2) NOT IN ('15', '40', '50', '55', '60')
