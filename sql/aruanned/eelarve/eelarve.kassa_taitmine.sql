...
WITH qryEelarve AS (
  ...
  FROM (
         ...
         WHERE j.rekvid = (CASE WHEN l_kond = 1
           THEN j.rekvid
                           ELSE l_rekvid END)
               AND j.rekvid IN (SELECT rekv_id
                                FROM get_asutuse_struktuur(l_rekvid))

               AND j1.deebet LIKE '100%' AND j1.deebet NOT LIKE '100080%'
         ...
         WHERE j.rekvid = (CASE WHEN l_kond = 1
           THEN j.rekvid
               ...
               AND j.rekvid IN (SELECT rekv_id
                                FROM get_asutuse_struktuur(l_rekvid))
               AND j1.kreedit LIKE '100%' AND j1.kreedit NOT LIKE '100080%'
               AND j.kpv <= make_date(year(l_kpv) - 1, 12, 31)
       ) qry_100
  ...
  FROM (
         ...
         WHERE j.rekvid = (CASE WHEN l_kond = 1
           THEN j.rekvid
               ...
               AND j.rekvid IN (SELECT rekv_id
                                FROM get_asutuse_struktuur(l_rekvid))
               AND (LEFT(j1.deebet, 3) = '100' OR LEFT(j1.deebet, 6) = '999999') AND (LEFT(j1.kood5, 4) = '3502')
               AND YEAR(j.kpv) = YEAR(l_kpv)
               AND j.kpv <= l_Kpv
         UNION ALL
         SELECT
         ...
         WHERE j.rekvid = (CASE WHEN l_kond = 1
           THEN j.rekvid
               ...
               AND j.rekvid IN (SELECT rekv_id
                                FROM get_asutuse_struktuur(l_rekvid))
               AND (LEFT(j1.deebet, 6) = '710000') AND (LEFT(j1.kood5, 4) = '3502')
               AND YEAR(kpv) = YEAR(l_kpv)
               AND j.kpv <= l_kpv
  ...
  WHERE j.rekvid = (CASE WHEN l_kond = 1
    THEN j.rekvid
        ...
        AND j.kpv <= l_kpv AND YEAR(kpv) = YEAR(l_kpv)
  UNION ALL
  --2586
  SELECT
    j.rekvid              AS rekv_id,
  ...
  WHERE aasta = YEAR(l_kpv)
        AND e.rekvid = (CASE WHEN l_kond = 1
    THEN e.rekvid
                        ELSE l_rekvid END)
...
SELECT
  ...
  'Likviidsete varade muutus (+ suurenemine, - vahenemine) Saldo seisuga ' || l_kpv :: TEXT :: VARCHAR(254) AS nimetus,
  sum(eelarve)
...
WHERE e.tegev IS NOT NULL AND NOT empty(tegev)
