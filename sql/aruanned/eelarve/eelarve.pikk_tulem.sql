...
FROM (
       ...
       WHERE val(left(ltrim(rtrim(q.konto)), 1)) >= 3 AND val(left(q.konto, 1)) < 9
       UNION ALL
       ...
       FROM qrySaldo q
         LEFT OUTER JOIN com_kontoplaan l ON ltrim(rtrim(q.konto)) = ltrim(rtrim(l.kood))
       WHERE val(left(konto, 1)) >= 9 AND val(left(konto, 3)) <= 920
       UNION ALL
       ...
       FROM qrySaldo q
         LEFT OUTER JOIN com_kontoplaan l ON ltrim(rtrim(q.konto)) = ltrim(rtrim(l.kood))
       WHERE val(left(konto, 3)) > 920 AND val(left(konto, 2)) < 93
       UNION ALL
       -- val(left(konto,2)) >= 93
       SELECT
         q.rekvid                       AS rekv_id,
       ...
       SELECT
         q.rekvid                       AS rekv_id,
         '38114',
         'Kasum/kahjum masinate ja seadmete müügist',
         coalesce(kr) - coalesce(db, 0) AS summa
       FROM qrySaldo q
         LEFT OUTER JOIN com_kontoplaan l ON ltrim(rtrim(q.konto)) = ltrim(rtrim(l.kood))
       WHERE konto LIKE '38114%'
       UNION ALL
       -- 38115
       SELECT
         q.rekvid                       AS rekv_id,
       ...
       FROM qrySaldo q
         LEFT OUTER JOIN com_kontoplaan l ON ltrim(rtrim(q.konto)) = ltrim(rtrim(l.kood))
       WHERE val(left(konto, 1)) >= 4 AND val(left(konto, 2)) <= 64
       UNION ALL
       -- 4
       SELECT
         q.rekvid                       AS rekv_id,
       ...
       SELECT
         q.rekvid                       AS rekv_id,
         '55110',
         'Kinnistute, hoonete, ruumide majandamiskulud (va kinnisvarainvesteeringud)',
         coalesce(kr) - coalesce(db, 0) AS summa
       FROM qrySaldo q
         LEFT OUTER JOIN com_kontoplaan l ON ltrim(rtrim(q.konto)) = ltrim(rtrim(l.kood))
       WHERE konto LIKE '55110%'
       UNION ALL
       -- 55112
       SELECT
         q.rekvid                       AS rekv_id,
       ...
       FROM qrySaldo q
         LEFT OUTER JOIN com_kontoplaan l ON ltrim(rtrim(q.konto)) = ltrim(rtrim(l.kood))
       WHERE konto LIKE '55112%'
       UNION ALL
       -- 55113
       SELECT
         q.rekvid                       AS rekv_id,
         '55113',
         'Üürile ja rendile antud kinnistute, hoonete, ruumide majandamiskulud (va kinnisvarainvesteeringud)',
         coalesce(kr) - coalesce(db, 0) AS summa
       FROM qrySaldo q
         LEFT OUTER JOIN com_kontoplaan l ON ltrim(rtrim(q.konto)) = ltrim(rtrim(l.kood))
       WHERE konto LIKE '55113%'
       UNION ALL
       -- 5512
       SELECT
         q.rekvid                       AS rekv_id,
       ...
       FROM qrySaldo q
         LEFT OUTER JOIN com_kontoplaan l ON ltrim(rtrim(q.konto)) = ltrim(rtrim(l.kood))
       WHERE val(left(konto,1)) >= 3 and val(left(konto,2)) <= 64
       UNION ALL
       -- 65
       SELECT
         q.rekvid                       AS rekv_id,
       ...
       FROM qrySaldo q
         LEFT OUTER JOIN com_kontoplaan l ON ltrim(rtrim(q.konto)) = ltrim(rtrim(l.kood))
       WHERE val(left(konto,1)) >= 3 and val(left(konto,1)) <= 6
       UNION ALL
       -- 7
       SELECT
         q.rekvid                       AS rekv_id,
       ...
       FROM qrySaldo q
         LEFT OUTER JOIN com_kontoplaan l ON ltrim(rtrim(q.konto)) = ltrim(rtrim(l.kood))
       WHERE val(left(ltrim(rtrim(konto)),1)) >= 3 and val(left(ltrim(rtrim(konto)),1)) <= 7
       UNION ALL
       -- 9
       SELECT
         q.rekvid                       AS rekv_id,
...
SELECT
  ...
  konto,
  nimetus
FROM eelarve.pikk_tulem('2018-06-30' :: DATE, 63, 1)
