     ...
     qrySaldo AS (
         ...
         GROUP BY rekvid
             ...
             SELECT s.rekvid,
                  ...
                  'Rahavood põhitegevusest' AS grupp,
                  'Korrigeerimised' AS all_grupp,
                  'Üle antud mitterahaline sihtfinantseerimine' AS nimetus,
                  sum(db) - sum(kr) AS summa
             FROM qrySaldoAndmik s
             WHERE s.aasta = year(l_kpv)
                      AND s.kuu = month(l_kpv)
             ...
             SELECT e.rekvid,
                  ...
                  coalesce(sum(db) FILTER (WHERE konto IN ('103500', '103540', '103556', '103557')), 0) -
                  (coalesce(sum(kr) FILTER (WHERE konto LIKE '1035%' ), 0) -
                   coalesce(sum(kr) FILTER (WHERE konto IN ('103500', '103540', '103556', '103557')), 0))
             FROM eelmiseSaldoAndmik e
             GROUP BY rekvid
             UNION ALL
             SELECT s.rekvid,
             ...
             SELECT s.rekvid,
                  ...
                  coalesce(sum(db - kr)
                           ...
                           0) -
                  coalesce(sum(db - kr)
                               FILTER (WHERE left(konto, 4) IN ('1036', '1536') AND kuu = month(l_kpv) AND
                                             aasta = year(l_kpv)),
                           0) AS summa
             FROM eelarve.saldoandmik s
             WHERE rekvid = (CASE
...
--         Jooksva per saldoandmikust (Konto 155* kreedit (RV 01)) - (Sum: Konto 155* deebet (RV 01)) +(Sum: Konto 156* kreedit (RV 01)) -
             ...
             FROM eelmiseSaldoAndmik s
             WHERE (
                          left(konto, 6) IN ('201010') OR
                          left(konto, 5) IN ('2501')
                      )
             GROUP BY rekvid
             UNION ALL
-- -Jooksva per saldoandmikust (Sum: Konto 154* kreedit (RV 01)) - (Sum: Konto 154* deebet (RV 01))
             SELECT rekvid,
                  ...
                  sum(kr - db) AS summa
             FROM qrySaldoAndmik S
             WHERE (LEFT(konto, 3) IN ('154') AND rahavoo = '01')
             GROUP BY rekvid
             UNION ALL
...
--(Jooksva per saldoandmikust (sum: kontod 10301* deebet + konto 15301* deebet) - (sum kontod 10301* kreedit + konto 15301* kreedit))
                          SELECT * FROM (
                                   WITH preQry AS (
                                       ...
                                       UNION ALL
                                       SELECT rekvid, -1 * (db - kr) AS summa
                                       FROM qrySaldoAndmik
                                       WHERE left(konto, 5) IN ('10301', '15301')
                                   ),
                                        qry AS (SELECT rekvid, sum(summa) AS summa FROM preQry GROUP BY rekvid)
                                   SELECT S.rekvid,
                                          '72'                                                                 AS konto,
                                          'Rahavood investeerimistegevusest'                                   AS grupp,
                                          'Laekunud põhivara müügist (v.a. finantsinvesteeringud ja osalused)' AS all_grupp,
...
--Jooksva per saldoandmikust (Sum: Konto 1032* kreedit (RV 02) + sum konto 1532* kreedit (RV 02)) - (Sum: Konto 1032* deebet (RV 02)+ sum konto 1532* deebet (RV 02))
             ...
             SELECT S.rekvid,
                  ...
                  sum(kr - db) AS summa
             FROM qrySaldoAndmik S
             WHERE LEFT(konto, 4) IN ('1032', '1532') AND rahavoo = '02'
             GROUP BY rekvid
             UNION ALL
...
--             + (Eelmise per saldoandmikust (konto 103856, 103857, 1537 deebet miinus kreedit)
             SELECT * FROM (
                      WITH osa_1 AS (
                          ...
                          GROUP BY rekvid
                              UNION ALL
                              SELECT rekvid, -1 * sum(db - kr) AS summa
                              FROM qrySaldoAndmik
                              WHERE (konto IN ('103856', '103857')
                           ...
                           osa_2 AS (
                               ...
                               GROUP BY rekvid
                                   UNION ALL
                                   SELECT rekvid, sum(db - kr) AS summa
                                   FROM eelmiseSaldoAndmik
                                   WHERE (konto IN ('103856', '103857')
                                   OR left(konto, 4) = '1537')
                                   GROUP BY rekvid
...
-- ((sum 1032* deebet RV 22 - sum 1032* kreedit RV 22 + sum 1532* deebet RV 22 - sum 1532* kreedit RV 22)) + ((konto 101900 deebet RV 21 - konto 101900 kreedit RV 21))
                           ...
                           preReport AS (
                               ...
                               GROUP BY rekvid
                                   ...
                                   GROUP BY rekvid
                                   UNION ALL
                                   SELECT rekvid, -1 * sum(summa) AS summa
                                   FROM osa_3
                                   GROUP BY rekvid
                                   UNION ALL
                                   SELECT rekvid, sum(summa) AS summa
...
--Jooksva per saldoandmikust (Sum konto 2081* kreedit (RV 06) - konto 2081* deebet (RV 06) + sum konto 2581* kreedit (RV 06) - konto 2581* deebet (RV 06)
             SELECT S.rekvid,
                  ...
                  sum(kr - db) AS summa
             FROM qrySaldoAndmik S
             WHERE LEFT(konto, 4) IN ('2081', '2581') AND rahavoo = '06'
             GROUP BY rekvid
             UNION ALL
                          ...
                          sum
                              (
                                      ...
                                      FILTER
                                          (
                                          WHERE (LEFT(konto
                                                     , 4) IN ('1537') OR konto IN ('103856', '103857')
                                                    )
                                          AND
                                                kuu
                                                    =
                      ...
                      (
                          sum
                              (
                                      ...
                                      FILTER
                                          (
                                          WHERE (konto IN ('103856', '103857') OR konto LIKE '1537%'
                                                    )
                                          AND
                                                kuu
                                                    =
                      ...
                      (
                          sum
                              (
                                      ...
                                      FILTER
                                          (
                                          WHERE (konto LIKE '1%' AND rahavoo IN ('15', '16')
                                                    )
                                          AND
                                                kuu
                                                    =
                      ...
                      (
                          sum
                              (
                                      ...
                                      FILTER
                                          (
                                          WHERE konto LIKE '2%' AND rahavoo IN ('15', '16', '35', '36'
                                          )
                                          AND
                                                kuu
                                                    =
...
FROM ( --     = korrigeeritud tegevustulem (3) + käibevarade muutus (4) + kohustuste muutus (5)
         ...
         GROUP BY rekvid
             ...
             SELECT rekvid,
                  ...
                  'Põhitegevusega seotud kohustuste netomuutus' AS all_grupp,
                  '50' AS konto,
                  'Põhitegevusega seotud kohustiste netomuutus' AS nimetus,
                  sum(summa) AS summa
             FROM qrySaldo S
             WHERE S.konto = '5'
             GROUP BY rekvid
