...
AS
$BODY$
WITH params AS (SELECT l_kpv1              AS kpv1,
                       l_kpv2              AS kpv2,
     ...
     rekv_ids AS (SELECT rekv_id
                  ...
                  WHERE rekv_id = CASE
                                      WHEN l_kond = 1
                                          THEN rekv_id
                                      ELSE l_rekvid END),
     pv_kaardid AS (SELECT l.id                                                                              AS pv_kaart_id,
                           l.rekvid,
                           (SELECT soetmaks FROM libs.get_pv_kaart_jaak(l.id, params.kpv1))                  AS alg_soetmaks,
                           coalesce((SELECT kulum FROM libs.get_pv_kaart_jaak(l.id, params.kpv1)),
                                    0)                                                                       AS alg_kulum,
                           CASE
                               WHEN l.status = 2 THEN 0
                               ELSE (SELECT soetmaks
                                     FROM libs.get_pv_kaart_jaak(l.id, params.kpv2 + 1)) END                 AS lopp_soetmaks,
                           CASE
                               WHEN l.status = 2 THEN 0
     ...
     po AS (SELECT *
            FROM docs.pv_oper pv,
                 params
            WHERE pv_kaart_id IN (SELECT pv_kaart_id
                                  FROM pv_kaardid)
              AND kpv >= params.kpv1
              AND kpv <= params.kpv2),
     po_rv11 AS (SELECT po.pv_kaart_id                AS pv_kaart_id,
                        sum(po.summa)::NUMERIC(12, 2) AS kb_kulum_rv11
                 FROM po
                 WHERE po.liik = 2
                   AND (coalesce(po.kood3, '11') = '11' OR empty(po.kood3))
                 GROUP BY pv_kaart_id),
     qryKaibed AS (SELECT pv_kaart_id,
                   ...
                   FROM (SELECT l.pv_kaart_id     AS pv_kaart_id,
                         ...
                         WHERE po.liik IN (1, 4)
                           AND COALESCE(po.kood3, '') = '29') kaibed
                   GROUP BY pv_kaart_id)
SELECT pk.kood::VARCHAR(20),
       ...
       coalesce(pk.alg_kulum,0):: NUMERIC(12, 2),
       -- pv osa
       coalesce(kb.kb_pv_rv01,0):: NUMERIC(12, 2),    -- Soetused ja parendused
       coalesce(kb.kb_pv_rv19,0):: NUMERIC(12, 2),    -- Saadud mitterahaline sihtfinantseerimine
       coalesce(kb.kb_pv_rv14,0):: NUMERIC(12, 2),    -- Üleviimine kinnisvarainvesteeringute grupist materiaalse põhivara gruppi
       coalesce(kb.kb_pv_rv13,0):: NUMERIC(12, 2),    -- Üleviimine materiaalse põhivara grupist kinnisvarainvesteeringute gruppi
       coalesce(kb.kb_pv_rv15,0):: NUMERIC(12, 2),    -- Varade mitterahalised siirded (üleandmine)
       coalesce(kb.kb_pv_rv16,0):: NUMERIC(12, 2),    -- Varade mitterahalised siirded (saamine)
       coalesce(kb.kb_pv_rv23,0):: NUMERIC(12, 2),    -- Ümberklassifitseerimine
       coalesce(kb.kb_pv_rv17,0):: NUMERIC(12, 2),    -- Varade üleandmine mitterahalise sissemaksena netovarasse
       coalesce(kb.kb_pv_rv24,0):: NUMERIC(12, 2),    -- Mitterahaline sihtfinantseerimine (üleandmine)
       coalesce(kb.kb_pv_rv29,0):: NUMERIC(12, 2),    -- Muud mitterahalised kanded varadega
       coalesce(kb.kb_pv_rv21,0):: NUMERIC(12, 2),    -- Varade ümberhindlus
       coalesce(kb.kb_pv_rv02,0):: NUMERIC(12, 2),    -- Müüdud põhivara
       coalesce(kb.kb_pv_rv12,0):: NUMERIC(12, 2),    -- Varade mahakandmine

       coalesce(pk.lopp_soetmaks,0):: NUMERIC(12, 2), -- Soetusmaksumus perioodi lõpus
       -- kulum osa
       coalesce(kb.kb_kulum_rv11,0):: NUMERIC(12, 2), -- Varade kulum ja allahindlus
       coalesce(kb.kb_kulum_rv02,0):: NUMERIC(12, 2), -- Müüdud põhivara
       coalesce(kb.kb_kulum_rv12,0):: NUMERIC(12, 2), -- Varade mahakandmine
       coalesce(kb.kb_kulum_rv14,0):: NUMERIC(12, 2), -- Üleviimine kinnisvarainvesteeringute grupist materiaalse põhivara gruppi
       coalesce(kb.kb_kulum_rv13,0):: NUMERIC(12, 2), -- Üleviimine materiaalse põhivara grupist kinnisvarainvesteeringute gruppi
       coalesce(kb.kb_kulum_rv15,0):: NUMERIC(12, 2), -- Varade mitterahalised siirded (üleandmine)
       coalesce(kb.kb_kulum_rv16,0):: NUMERIC(12, 2), -- Varade mitterahalised siirded (saamine)
       coalesce(kb.kb_kulum_rv17,0):: NUMERIC(12, 2), -- Varade üleandmine mitterahalise sissemaksena netovarasse
       coalesce(kb.kb_kulum_rv24,0):: NUMERIC(12, 2), -- Mitterahaline sihtfinantseerimine (üleandmine)
       coalesce(kb.kb_kulum_rv29,0):: NUMERIC(12, 2), -- Muud mitterahalised kanded varadega

       coalesce(lopp_kulum,0):: NUMERIC(12, 2),
...
/*SELECT *
FROM (SELECT *
      FROM docs.pv_rv_kaibe_aruanne('2024-01-01', '2024-08-31' :: DATE, 130, 1, '{
        "konto": "155500"
      }'::JSONB)) qry
