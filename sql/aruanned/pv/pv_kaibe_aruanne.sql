                    ...
                    FROM
                        (
                            SELECT
                                ...
                                (
                                    SELECT
                                        soetmaks
                                    FROM
                                        libs.get_pv_kaart_jaak(l.id, l_kpv1)
                                )                                AS alg_soetmaks,
                                (
                                    SELECT
                                        kulum
                                ...
                                (CASE
                                     ...
                                     ELSE 0 END)::NUMERIC(12, 2) AS db_kulum,
                            ...
                            SELECT
                                ...
                                0                                                                                AS db_kulum,
                                sum(-1 * summa)
                                FILTER (WHERE po.liik IN (3) and coalesce(po.kood3, '') = '12')::NUMERIC(12, 2)  AS kr_soetmaks,
                                sum(-1 * summa)
                                FILTER ( WHERE po.liik IN (2) and coalesce(po.kood3, '') = '12')::NUMERIC(12, 2) AS kr_kulum
                            FROM
                                docs.doc                    D
                                    INNER JOIN docs.pv_oper po
                                               ON po.parentid = D.id
                            ...
                            WHERE
                              ...
                              AND (
                                      l.properties :: JSONB ->> 'mahakantud')::DATE IS NOT NULL
                              AND (
                                      l.properties :: JSONB ->> 'mahakantud')::DATE >= l_kpv1
                              AND (
                                      l.properties :: JSONB ->> 'mahakantud')::DATE <= l_kpv2
                            GROUP BY
                                po.pv_kaart_id,
                                l.id
                        ) qry
...
/*
SELECT *
FROM docs.pv_kaibe_aruanne('2023-01-01', '2023-12-31' :: DATE,130)
where kood like '01247-07R%'
...
select * from ou.rekv where nimetus ilike '%tuluke%'
*/