...
--    WHERE qry.jaak > 0
    ...
    -- счета, по которым сделан расчет маловероятных, но потом найдена их оплата
    vigasedArved as (
                  select
                      d.id as doc_id,
                      d.docs_ids,
                  ...
                  WHERE
                    ...
                    and (a.properties ->> 'ebatoenaolised_1_id' is not null
                      or a.properties ->> 'ebatoenaolised_2_id' is not null)
              ),

    ArvestatudPaevaraamatus AS (
                  select
                      sum(summa) as summa,
                      doc_id
                  from
