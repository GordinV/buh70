...
WITH
    ...
    params as (
                      select
                          jsonb_build_object('laps_id', to_jsonb(array_agg(lj.laps_id))) as laps_ids
                      from
                          laste_jaak lj
                  ),
    raamatu_jaak as (
                      SELECT
                          ...
                          coalesce(la.arveldus, false)                          as kas_vastutav_isik
                      FROM
                          params                                                                              p,
                          docs.kaibeasutusandmik('10300029', null::integer, l_kpv::date, l_kpv::date,
                                                 l_rekvid, '%', 1, jsonb_build_object('laps_id', p.laps_ids)) rep
                              left outer join lapsed.vanem_arveldus                                           la
                                              on la.parentid = rep.laps_id and la.rekvid = rep.rekv_id and
                                                 la.asutusid = rep.asutus_id

                      where
                            ((rep.alg_saldo + deebet - kreedit) <> 0
                                -- and not coalesce(la.arveldus, false)
                                )
                        and la.rekvid = l_rekvid
...
SELECT
    ...
    lj.rekvid::integer,
    case
        when lj.jaak <> coalesce(rj.laps_jaak_kokku, 0) then true
        when rj.laps_jaak_kokku <> 0 and not rj.kas_vastutav_isik then true
        else false end                                  as kas_viga,
    null::text
from
    laste_jaak                       lj
...
where
  ...
  and (case
    ...
    --and      (lj.jaak <> 0 and rj.laps_jaak_kokku <> 0)
--  and coalesce(rj.laps_jaak_kokku, 0) <> 0
--  and (
--          laps_vastisik_jaak_kogus > 1
--              or
--          not kas_vastutav_isik
--          )
...
select *
from
    lapsed.saldo_aruanne(69, '2025-05-31'::DATE)
where
