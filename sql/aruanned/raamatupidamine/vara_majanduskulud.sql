    ...
    SELECT coalesce(l_params::JSONB ->> 'konto', '')::TEXT || '%'                                                                                         AS konto,
     ...
     qry_docs AS (
         ...
         WHERE d.status < 3
           ...
           AND left(a1.konto, 6) IN (SELECT unnest(p.kontod))
     )
SELECT konto,
       konto_nimetus,
       objekt,
...
ORDER BY kas_kulud DESC, konto, idx, asutus, partner
...
SELECT *
FROM docs.vara_majanduskulud('2024-01-01'::DATE, '2024-02-28'::DATE, 66::INTEGER, NULL::JSONB)
