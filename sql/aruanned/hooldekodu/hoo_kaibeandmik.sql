...
CREATE OR REPLACE FUNCTION hooldekodu.hoo_kaibeandmik(l_kpv1 DATE, l_kpv2 DATE, l_rekvid INTEGER,
    ...
    RETURNS TABLE (
        ...
        taskuraha_kov_alg  NUMERIC(14, 2),
        ...
        taskuraha_kov_lopp NUMERIC(14, 2)
    )
AS
$BODY$
SELECT *
FROM (
         ...
         SELECT qry.isik_id,
                ...
                sum(taskuraha_kov_alg)                                          AS taskuraha_kov_alg,
                ...
                sum(taskuraha_kov_laek)                                         AS taskuraha_kov_laek,
                ...
                sum(taskuraha_kov_arv)                                          AS taskuraha_kov_arv,
                ...
                sum(taskuraha_kov_alg + taskuraha_kov_laek - taskuraha_kov_arv) AS taskuraha_kov_lopp
         FROM (
                  SELECT a.isik_id,
                         ...
                         a.vara          AS vara_alg,
                         a.muud          AS muud_alg,
                         a.taskuraha_kov AS taskuraha_kov_alg,
                         0               AS pension_85_laek,
                         0               AS pension_15_laek,
                         0               AS toetus_laek,
                         0               AS vara_laek,
                         0               AS muud_laek,
                         0               AS taskuraha_kov_laek,
                         0               AS pension_85_arv,
                         0               AS pension_15_arv,
                         0               AS toetus_arv,
                         0               AS vara_arv,
                         0               AS muud_arv,
                         0               AS taskuraha_kov_arv
                  FROM algsaldo a
                  UNION ALL
                  SELECT isik_id,
                         0               AS pension_85_alg,
                  ...
                  SELECT isik_id,
                         ...
                         a.vara          AS vara_arv,
                         a.muud          AS muud_arv,
                         a.taskuraha_kov AS taskuraha_kov_arv
                  FROM arvestus a) qry
         GROUP BY isik_id
     ) report
WHERE pension_85_alg <> 0
   OR pension_15_alg <> 0
   OR pension_15_arv <> 0
   OR pension_15_laek <> 0
   OR toetus_alg <> 0
   OR vara_alg <> 0
   OR muud_alg <> 0
   OR taskuraha_kov_alg <> 0
   OR pension_85_laek <> 0
   OR pension_15_laek <> 0
   OR toetus_laek <> 0
   OR vara_laek <> 0
   OR muud_laek <> 0
   OR taskuraha_kov_laek <> 0
   OR pension_85_arv <> 0
   OR pension_15_arv <> 0
   OR toetus_arv <> 0
   OR vara_arv <> 0
   OR muud_arv <> 0
   OR taskuraha_kov_arv <> 0
   OR pension_85_lopp <> 0
   OR pension_15_lopp <> 0
   OR toetus_lopp <> 0
   OR vara_lopp <> 0
   OR muud_lopp <> 0
   OR taskuraha_kov_lopp <> 0
    ;
$BODY$
    LANGUAGE SQL
    VOLATILE
...
*/
SELECT *
FROM hooldekodu.hoo_kaibeandmik('2023-07-01', '2023-07-31':: DATE, 132, 1)
