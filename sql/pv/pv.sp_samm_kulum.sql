    ...
    FOR v_pv_kaards IN
        ...
        LOOP
            ...
            SELECT qry.result,
                   ...
                   qry.summa
                   INTO v_tulemus
            FROM docs.sp_calc_kulum(v_pv_kaards.id :: INTEGER, l_kpv::DATE) qry;
            ...
            THEN
                ...
                FROM (SELECT l_pv_oper_id              AS id,
                             l_kpv                     AS kpv,
                             l_nomid                   AS nomid,
                             2                         AS liik,
...
/*
 ...
 SELECT pv.id, pv.kpv, pv.summa, docs.sp_samm_kulum(1, json_build_object(
                'ids', array[pv.pv_kaart_id],
                'kpv', pv.kpv)) as tulemus
from docs.pv_oper pv,
   (select pv_kaart_id, kpv from docs.pv_oper po where id = 88461) as po
where pv.pv_kaart_id = po.pv_kaart_id and pv.liik = 2 and pv.kpv > po.kpv
