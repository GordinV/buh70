DROP FUNCTION IF EXISTS ladu.gen_lausend_sarv(INTEGER, INTEGER);

CREATE OR REPLACE FUNCTION ladu.gen_lausend_sarv(IN tnId INTEGER, IN userId INTEGER, OUT error_code INTEGER,
                                                 OUT result INTEGER, OUT error_message TEXT)
AS
$BODY$
DECLARE
  lcDbKonto      VARCHAR(20) = '103000';
...
GRANT EXECUTE ON FUNCTION ladu.gen_lausend_sarv(INTEGER, INTEGER) TO ladukasutaja;
GRANT EXECUTE ON FUNCTION ladu.gen_lausend_sarv(INTEGER, INTEGER) TO dbkasutaja;
GRANT EXECUTE ON FUNCTION ladu.gen_lausend_sarv(INTEGER, INTEGER) TO dbpeakasutaja;
