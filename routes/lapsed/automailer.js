        ...
        let sql = `WITH
                       doc AS (
                                  WITH
                                      params AS (
                                                    SELECT
                                                        make_date(year(current_date), month(current_date), 1) AS kpv2,
                                                        case
                                                            when month(current_date) = 1
                                                                then make_date(year(current_date) - 1, 12, 01)
                                                            else gomonth(
                                                                    make_date(year(current_date), month(current_date), 1),
                                                                    -1) end                                   AS kpv1
                                                ),
                                      kas_lubatud AS (
                                                    SELECT
                                      ...
                                      docs AS (
                                                    ...
                                                    FROM
                                                        docs.arv                                  a
                                                            ...
                                                            LEFT OUTER JOIN lapsed.vanem_arveldus va
                                                                            ON l.id = va.parentid AND
                                                                               va.asutusid = a.asutusid AND
                                                                               va.rekvid = a.rekvid
                                                    ...
                                                    WHERE
                                                      ...
                                                      AND d.doc_type_id IN (
                                                                               SELECT id
                                                                               FROM libs.library
                                                                               WHERE library.library = 'DOK' AND kood = 'ARV'
                                                                           )
                                                      AND va.kas_email
                                                      AND (va.properties ->> 'email_alates' is null or
                                                           (va.properties ->> 'email_alates')::date <= current_date)
