...
exports.post = async (req, res) => {
    ...
    const taskName = 'arvestaEbatoenaolised';
    ...
    console.log(ids, execDate, user.asutusId);

    let lastDocId = 1;
    try {

        doc.setDocumentId(ids[0]);
        let data = await doc.executeTask(taskName, [ids.join(','), user.asutusId, execDate]);
        tulemused = data;
