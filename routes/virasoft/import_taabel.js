...
module.exports = async (req, res) => {
    ...
    console.log('import taabel start .')

    //async (file, mimeType) => {
    const Doc = require('./../../classes/DocumentTemplate');
    const Document = new Doc('palk_taabel', null, null, null, 'palk');
...
//    console.log('req', JSON.stringify(req));
    ...
    console.log('import taabel  got file', file, params);
    ...
    try {
        ...
        if (rows.length && !returnObject.error_message) {
            ...
            if (response && response.data && response.data.length > 0) {
                ...
                returnObject = {
                    error_message: `Kokku leidsin ${rows.length} read, salvestatud kokku: ${saved}`,
                    result: saved,
                    data: returnData
                };
