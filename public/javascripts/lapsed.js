	            ...
	            if (yksus) {
	                nomData = (yksus.teenused ? yksus.teenused : []).map(function (nom) {
	                    ...
	                    var row = self.libs['nomenclature'].find(function (lib) {
	                        return lib.id === nom.id;
	                    });
	                    console.log('row', row);
	                    if (row) {
	                        var teenuseNimetus = row.nimetus ? row.nimetus + ' (' + Number(nom.hind).toFixed(2) + ') ' : '';
	                        return _extends({}, row, { nimetus: teenuseNimetus, id: Number(!nom.id || nom.id == NaN ? 0 : nom.id) });
	                    }
	                }).sort(function (a, b) {
	                    return a.kood.localeCompare(b.kood);
	                });
	                console.log('nomData 2', yksus);
	            }
